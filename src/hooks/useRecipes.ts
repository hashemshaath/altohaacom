  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) throw error;
      // Fallback: try by UUID if slug lookup returned nothing
      if (!data) {
        ({ data, error } = await supabase.from("recipes").select(RECIPE_FIELDS).eq("id", slug).maybeSingle());
        if (error) throw error;
      }
      if (!data) return null;
