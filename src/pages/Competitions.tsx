            ...
            <div className="flex gap-1 overflow-x-auto scrollbar-none">
              {TAB_FILTERS.map((tab) => (
                <button
                  ...
                  className={`relative inline-flex shrink-0 items-center gap-1 rounded-full px-3 py-1.5 text-xs font-bold uppercase tracking-wider transition-all duration-200 touch-manipulation active:scale-95 ${
                    activeTab === tab
                ...
                >
                  ...
                  <span className={`ms-0.5 inline-flex h-4 min-w-[16px] items-center justify-center rounded-full px-1 text-xs font-black tabular-nums ${
                    activeTab === tab
                  ...
                  <Link key={comp.id} to={`/competitions/${comp.id}`}>
                    <Card className="group overflow-hidden rounded-2xl border-border/20 transition-all hover:shadow-md hover:-translate-y-0.5">
                      <CardContent className="flex items-center gap-4 p-3.5">
                        ...
                        <Badge variant={status.status === "in_progress" ? "default" : "outline"} className="shrink-0 text-xs">
                          {status.label}
                        </Badge>
                      </CardContent>
                    </Card>
