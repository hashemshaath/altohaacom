          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              {data.counts.unread > 0 && (
                <Badge className="gap-1 text-xs sm:text-xs px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl animate-pulse shadow-sm shadow-primary/15">
                  <Bell className="h-3 w-3" />
                  {data.counts.unread}
                </Badge>
              )}
              <div className="hidden sm:flex items-center gap-1 rounded-xl border border-border/30 bg-card/60 backdrop-blur-sm p-1">
                <Badge variant="secondary" className="text-xs gap-1 cursor-default rounded-xl">
                  <MessageSquare className="h-3 w-3" />
                  {data.counts.all}
                </Badge>
                {data.chatGroups.length > 0 && (
                  <Badge variant="outline" className="text-xs gap-1 cursor-default rounded-xl">
