        ...
        <CardContent className="pt-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <Button variant="outline" size="sm" className="text-xs gap-1.5 col-span-2" onClick={async () => {
              if (!pageId) return;
              const { data } = await supabase.from("bio_subscribers").select("email, name, subscribed_at").eq("page_id", pageId).order("subscribed_at", { ascending: false }).limit(QUERY_LIMIT_LARGE);
              if (!data?.length) return;
