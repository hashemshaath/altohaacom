...
import { Trophy, Medal, Award, ArrowLeft, User, ChefHat, Sparkles, Filter } from "lucide-react";
...
const PODIUM_CONFIG = [
  { rank: 2, icon: Medal, color: "text-gray-400", bgColor: "bg-muted", borderColor: "border-muted-foreground/20", height: "h-28 md:h-32", medal: "Silver", medalAr: "فضي" },
  { rank: 1, icon: Trophy, color: "text-yellow-500", bgColor: "bg-chart-4/10", borderColor: "border-chart-4/30", height: "h-36 md:h-40", medal: "Gold", medalAr: "ذهبي" },
  { rank: 3, icon: Award, color: "text-amber-600", bgColor: "bg-chart-4/5", borderColor: "border-chart-4/20", height: "h-20 md:h-24", medal: "Bronze", medalAr: "برونزي" },
];
...
export default function CompetitionResults() {
  ...
  const filteredWinners = winners?.filter((w) =>
    categoryFilter === "all" ? true : w.category_id === categoryFilter
  );
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <SEOHead
        title={`${title} - ${isAr ? "النتائج" : "Results"}`}
        description={`Official results for ${title}`}
      />
      <Header />
        ...
        <div className="mb-8 text-center">
          <div className="mb-3 flex items-center justify-center gap-2">
            ...
            <Badge variant="outline" className="text-sm px-3">
              {isAr ? "النتائج الرسمية" : "Official Results"}
            </Badge>
            <Sparkles className="h-5 w-5 text-chart-4" />
          </div>
          <h1 className="mb-2 font-serif text-2xl font-bold md:text-3xl lg:text-4xl">{title}</h1>
          <p className="text-sm text-muted-foreground">
            {isAr ? "تهانينا لجميع الفائزين!" : "Congratulations to all winners!"}
          </p>
        </div>
        ...
        {topThree.length > 0 ? (
              ...
              {podiumOrder.map((winner, index) => {
                ...
                return (
                  <div key={winner.registration_id} className="flex flex-col items-center">
                    <Card className={`mb-2 w-28 sm:w-32 md:w-40 border ${config.borderColor} ${config.bgColor}`}>
                      <CardContent className="p-3 md:p-4 text-center">
                        <div className="relative mx-auto mb-2 h-14 w-14 md:h-18 md:w-18">
                          {winner.dish_image_url ? (
                            <img
                              src={winner.dish_image_url}
                              alt={winner.dish_name || "Dish"}
                              className="h-full w-full rounded-full object-cover ring-2 ring-offset-2 ring-border"
                            />
                          ) : (
                          ...
                          <div className="absolute -bottom-1 -end-1 flex h-7 w-7 items-center justify-center rounded-full bg-background shadow-sm border">
                            <Icon className={`h-4 w-4 ${config.color}`} />
                          </div>
                        </div>

                        <p className="mb-0.5 truncate text-xs md:text-sm font-medium">
                          {winner.dish_name || (isAr ? "طبق" : "Dish")}
                        </p>
                        ...
                        {winner.category_name && (
                          <Badge variant="outline" className="mt-1.5 text-[8px] md:text-[9px] h-4 px-1">
                            {isAr && winner.category_name_ar ? winner.category_name_ar : winner.category_name}
                          </Badge>
                        )}

                        <p className="mt-1.5 text-lg md:text-xl font-bold text-primary">
                          {winner.total_score.toFixed(1)}
                        </p>

                        <Badge className="mt-1 text-[9px]" variant="secondary">
                          {isAr ? config.medal : config.medal}
                        </Badge>
                    ...
                    {/* Podium Block */}
                    <div className={`flex w-20 md:w-28 items-center justify-center rounded-t-lg border border-b-0 ${config.bgColor} ${config.height}`}>
                      <span className="text-2xl md:text-3xl font-bold text-muted-foreground">{winner.rank}</span>
                    </div>
                  </div>
                );
              })}
          ...
          <Card className="mb-12">
            <CardContent className="py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {isAr ? "لا توجد نتائج حتى الآن" : "No results available yet"}
              </p>
            </CardContent>
          </Card>
        )}
        ...
        {others.length > 0 && (
          <div className="max-w-2xl mx-auto">
            <h2 className="mb-4 text-lg font-semibold flex items-center gap-2">
              ...
              <Badge variant="secondary" className="ms-1">{others.length}</Badge>
            </h2>
            <div className="space-y-2">
              {others.map((winner) => (
                <Card key={winner.registration_id} className="overflow-hidden">
