...
import { Trophy, Award, Star, ChefHat, TrendingUp, Target, BarChart3 } from "lucide-react";
...
export default function ChefPortfolio() {
  ...
  const { data: profile, isLoading: profileLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("profiles_public").select("user_id, full_name, username, avatar_url, job_title, country_code, bio, specialization").eq("user_id", targetUserId!).maybeSingle();
      return data;
    },
            ...
            {[
              ...
              <Card key={i} className="border-border/50">
                <CardContent className="flex items-center gap-3 p-4">
                  ...
                  <div>
                    <p className="text-xs uppercase font-bold tracking-wider text-muted-foreground">{stat.label}</p>
                      ...
                      {registrations.map((reg) => {
                        ...
                        return (
                          <div key={reg.id} className="flex items-center gap-3 rounded-xl border border-border/60 p-3 hover:bg-muted/30 transition-colors">
                            ...
                            <Badge variant={reg.status === "approved" ? "default" : "secondary"} className="text-xs shrink-0">
                              {reg.status}
                            </Badge>
                          </div>
                        );
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm flex items-center gap-2">
                    ...
                    {totalCerts > 0 && <Badge variant="secondary" className="text-xs">{totalCerts}</Badge>}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {certificates?.length ? (
                      ...
                      {certificates.slice(0, 5).map((cert) => (
                        <div key={cert.id} className="flex items-center gap-2 rounded-xl bg-muted/40 p-2.5">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-muted-foreground">{cert.type} • {cert.issued_at ? format(new Date(cert.issued_at), "MMM yyyy") : "—"}</p>
                          </div>
                        </div>
                      ))}
                    </div>
