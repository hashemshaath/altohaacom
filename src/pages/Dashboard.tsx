...
import { Trophy, Users, GraduationCap, Landmark, MessageSquare, ShoppingBag, Sparkles, Award, Star, UtensilsCrossed, HandHeart, AlertCircle, Megaphone, ClipboardList, ArrowRight, ChevronRight, Calendar, Zap, LucideIcon } from "lucide-react";
        ...
        <div className="flex items-start gap-4 sm:gap-5">
          ...
          <Link to={ROUTES.profileDashboard} className="shrink-0 hidden sm:block">
            <div className={`h-16 w-16 sm:h-[72px] sm:w-[72px] rounded-2xl p-[2.5px] shadow-xl transition-all duration-300 hover:scale-105 hover:shadow-primary/25 ${
              ...
              {avatarUrl ? (
                <img src={avatarUrl} alt="Profile photo" loading="lazy" className="h-full w-full rounded-[13px] object-cover bg-card" />
              ) : (
        ...
        <div className="flex gap-2.5 overflow-x-auto pb-2 sm:pb-0 sm:grid sm:grid-cols-5 lg:grid-cols-11 sm:overflow-visible scrollbar-none touch-manipulation" dir={isAr ? "rtl" : "ltr"}>
          {sections.map((s) => (
            <Link key={s.title} to={s.href} className="group shrink-0 sm:shrink" {...prefetchProps(s.href)}>
              <div className={`flex flex-col items-center gap-2 p-3.5 rounded-2xl border border-border/30 bg-card/60 backdrop-blur-sm transition-all duration-300 hover:shadow-lg hover:border-primary/20 hover:-translate-y-0.5 active:scale-[0.98] w-[78px] sm:w-auto ${s.glow}`}>
