              ...
              <CardContent className="p-6 space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <Badge variant="secondary" className="text-xs mt-1">{inv.domain_slug}</Badge>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {inv.product_images.slice(0, 4).map((url, i) => (
                      <div key={i} className="h-16 w-16 rounded-xl overflow-hidden border border-border/40">
                        <img src={url} alt={`Product image ${i + 1}`} className="h-full w-full object-cover" loading="lazy" decoding="async" />
                      </div>
                    ))}
                  </div>
                )}
          ...
          {responded.map(inv => (
            <Card key={inv.id} className="border-border/40">
              <CardContent className="p-4 flex items-center justify-between gap-4">
                ...
                <Badge variant={inv.status === "accepted" ? "default" : "destructive"} className="text-xs uppercase">
