        ...
        <div className="relative">
          {coverImage ? (
            <div className="h-56 md:h-72 overflow-hidden relative">
              <img src={coverImage} alt={`${orgName} cover`} loading="lazy" className="w-full h-full object-cover" loading="lazy" decoding="async" />
                            ...
                            <Link key={ex.id} to={`/exhibitions/${ex.slug}`} className="group block">
                              <Card className="hover:shadow-md transition-all border-border/40 hover:border-primary/30 rounded-2xl">
                                <CardContent className="p-3 flex items-center gap-4">
                                  {ex.cover_image_url && (
                                    <img src={ex.cover_image_url} alt={isAr && ex.title_ar ? ex.title_ar : ex.title} loading="lazy" className="h-16 w-24 rounded-xl object-cover shrink-0" loading="lazy" decoding="async" />
                                  )}
                                  <div className="flex-1 min-w-0">
                            ...
                            <Card className="overflow-hidden hover:shadow-md transition-all border-border/40 hover:border-primary/30 h-full rounded-2xl">
                              {ex.cover_image_url && (
                                <div className="relative h-36 overflow-hidden">
                                  <img src={ex.cover_image_url} alt={isAr && ex.title_ar ? ex.title_ar : ex.title} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" decoding="async" />
                  ...
                  {allSponsors.map((sponsor, i) => (
                    <Card key={i} className="border-border/40 rounded-2xl">
                      <CardContent className="p-4 flex items-center gap-3">
                        {sponsor.logo ? (
                          <img src={sponsor.logo} alt={sponsor.name} loading="lazy" className="h-10 w-10 rounded-xl object-contain bg-muted p-1" loading="lazy" />
                        ) : (
                    ...
                    <Link key={article.id} to={`/blog/${article.slug}`} className="group">
                      <Card className="overflow-hidden hover:shadow-md transition-all border-border/40 hover:border-primary/30 h-full rounded-2xl">
                        {article.featured_image_url && (
                          <img src={article.featured_image_url} alt={isAr && article.title_ar ? article.title_ar : article.title} loading="lazy" className="w-full h-36 object-cover" loading="lazy" decoding="async" />
                        )}
