...
export default function SupplierCompare() {
  ...
  const { data: selectedSuppliers = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("companies")
        .select("id, name, name_ar, type, logo_url, cover_image_url, description, description_ar, city, country_code, status, website, phone, email, specializations, is_verified, created_at")
        .in("id", selectedIds)
        .eq("status", "active");
      return data || [];
    },
            ...
            {selectedIds.map((id) => {
              ...
              return (
                <Badge key={id} variant="secondary" className="gap-2 py-1.5 px-3 text-sm">
                  {s?.logo_url ? (
                    <img loading="lazy" src={s.logo_url} className="h-4 w-4 rounded object-contain" alt={s.name || "Supplier"} />
                  ) : (
                  ...
                  {filteredPicker.slice(0, 12).map((s) => (
                    ...
                    >
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-muted">
                        {s.logo_url ? (
                          <img loading="lazy" src={s.logo_url} className="h-5 w-5 object-contain" alt={s.name || "Supplier"} />
                        ) : (
