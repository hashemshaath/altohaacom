  ...
  useEffect(() => {
    if (competition && existingCategories && existingCriteria && existingTypeAssignments && existingSupervisingBodies && existingJudges) {
      setData({
        ...
        registrationTaxName: (competition as Record<string, unknown>).registration_tax_name as string || "VAT",
        registrationTaxNameAr: (competition as Record<string, unknown>).registration_tax_name_ar as string || "ضريبة القيمة المضافة",
        allowedEntryTypes: (competition as Record<string, unknown>).allowed_entry_types as string[] || ["individual"],
        maxTeamSize: (competition as Record<string, unknown>).max_team_size as number || 5,
        minTeamSize: (competition as Record<string, unknown>).min_team_size as number || 2,
        linkType: (competition as Record<string, unknown>).link_type as string || "exhibition",
        linkedEntityId: (competition as Record<string, unknown>).linked_entity_id as string || null,
        linkedChefId: (competition as Record<string, unknown>).linked_chef_id as string || null,
        linkedTastingId: (competition as Record<string, unknown>).linked_tasting_id as string || null,
        seriesId: (competition as Record<string, unknown>).series_id as string || "",
