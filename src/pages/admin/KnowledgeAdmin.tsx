      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="rounded-2xl border border-border/40 bg-muted/30 p-1.5 h-auto">
          <TabsTrigger value="resources" className="gap-2 rounded-xl data-[state=active]:shadow-sm">
            ...
            {resources && <Badge variant="secondary" className="ms-1 text-xs h-4 px-1.5 rounded-md">{resources.length}</Badge>}
          </TabsTrigger>
                    ...
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          <Badge variant="outline" className="text-xs rounded-md">{resource.resource_type}</Badge>
                          ...
                          {(resource as any).category && (
                            <Badge variant="outline" className="text-xs rounded-md bg-muted/50">
            ...
            {references?.map(ref => (
              <Card key={ref.id} className="overflow-hidden rounded-2xl border-border/40 hover:shadow-lg hover:-translate-y-0.5 transition-all group">
                <div className="aspect-video relative overflow-hidden">
                  ...
                  />
                  <Badge className={cn("absolute top-3 end-3 rounded-xl", ratingColors[ref.rating || "good"])}>
                    {ref.rating}
                  </Badge>
                </div>
                <CardContent className="p-4">
                  ...
                  {ref.competition_category && (
                    <Badge variant="outline" className="mt-1.5 text-xs rounded-md">{ref.competition_category}</Badge>
                  )}
              ...
              <Card key={rubric.id} className="rounded-2xl border-border/40 hover:shadow-md hover:-translate-y-0.5 transition-all">
                <CardContent className="flex items-center gap-3 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      {rubric.competition_type && <Badge variant="outline" className="text-xs rounded-md">{rubric.competition_type}</Badge>}
                      {rubric.category_type && <Badge variant="secondary" className="text-xs rounded-md">{rubric.category_type}</Badge>}
                    </div>
                  </div>
                </CardContent>
              </Card>
