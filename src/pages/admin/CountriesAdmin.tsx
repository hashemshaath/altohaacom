      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="flex-wrap">
          ...
          <TabsTrigger value="countries">
            ...
            <Badge variant="secondary" className="ms-1.5 text-xs">{filtered.length}</Badge>
          </TabsTrigger>
                      ...
                      <TableRow key={c.id} className={`${!c.is_active ? "opacity-60" : ""} ${detailCountry?.id === c.id ? "bg-primary/5" : ""}`}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            {c.code_alpha3 && <Badge variant="secondary" className="font-mono text-xs">{c.code_alpha3.trim()}</Badge>}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div>
                            <p className="text-xs">{c.region || "—"}</p>
                            {c.continent && <p className="text-xs text-muted-foreground">{c.continent}</p>}
                          </div>
                        </TableCell>
                        <TableCell className="text-xs">
                          <span className="font-medium">{c.currency_symbol}</span> {c.currency_code.trim()}
                          {c.tax_rate ? <p className="text-xs text-muted-foreground">{c.tax_name} {c.tax_rate}%</p> : null}
                        </TableCell>
                        <TableCell>
                          <TooltipProvider>
                            <Tooltip>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <Badge variant="outline" className="text-xs font-mono">{c.code.trim()}</Badge>
                          <span className="text-xs text-muted-foreground">{c.currency_symbol} {c.currency_code.trim()}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5">
                        {c.is_featured && <Star className="h-3.5 w-3.5 text-chart-2 fill-chart-2" />}
            ...
            <ScrollArea className="flex-1 px-1">
              ...
              <TabsContent value="locale" className="space-y-4 mt-4">
                ...
                <div className="space-y-1.5">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">{isAr ? "مفصولة بفاصلة" : "Comma-separated language codes"}</p>
                </div>
                <Separator />
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1.5">
