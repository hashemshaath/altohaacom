...
const AdvertisingAdmin = forwardRef<HTMLDivElement>(function AdvertisingAdmin(_props, ref) {
  ...
  const { data: placements = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("ad_placements").select("id, name, name_ar, slug, placement_type, format, page_location, position, is_active, is_premium, sort_order, max_ads, width, height, base_cpm, base_cpc, base_cpv").order("sort_order").limit(QUERY_LIMIT_LARGE);
      if (error) throw handleSupabaseError(error);
      return data;
    },
  });
  ...
  const { data: packages = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("ad_packages").select("id, name, name_ar, tier, price, currency, duration_days, max_impressions, max_clicks, max_campaigns, is_active, sort_order, included_placements, features").order("sort_order").limit(QUERY_LIMIT_LARGE);
      if (error) throw handleSupabaseError(error);
      return data;
    },
  });
