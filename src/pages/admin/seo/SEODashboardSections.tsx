/**
 * Inline section renderers for SEO Dashboard — Vitals, Crawlers, Keywords, etc.
 */
...
import {
  Search, Globe, Eye, Smartphone, Monitor, Tablet,
  TrendingUp, RefreshCw, Send, BarChart3, ArrowUp, ArrowDown, Minus,
  AlertTriangle, CheckCircle2, Activity, Gauge, Zap,
  Bot, Target, FileSearch, Plus, Trash2, Shield,
  FileCode, Save, Route, Map,
} from "lucide-react";
import { CWV_THRESHOLDS, getVitalStatus, CHART_COLORS, PUBLIC_ROUTES, DEFAULT_ROBOTS_TXT } from "./seoDashboardTypes";
...
interface SectionProps {
  isAr: boolean;
  data: ReturnType<typeof import("./useSEODashboardData").useSEODashboardData>;
}

// ── Web Vitals ──
export const VitalsSection = memo(function VitalsSection({ isAr, data }: SectionProps) {
  const { vitalsAgg, pageVitals, trendData, deviceVitalsComparison, connectionDistribution, collectingVitals, handleCollectVitals } = data;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h3 className="text-base font-semibold flex items-center gap-2"><Gauge className="h-4 w-4 text-primary" />{isAr ? "مقاييس الأداء الأساسية" : "Core Web Vitals"}</h3>
          ...
          return (
            <Card key={metric} className="relative overflow-hidden">
              ...
              <CardContent className="p-4 pt-5">
                <p className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-1">{t.label}</p>
                {val != null ? (
                  <>
                    <p className={`text-2xl font-bold tabular-nums ${status === "good" ? "text-chart-2" : status === "needs-improvement" ? "text-chart-4" : "text-destructive"}`}>
                      {metric === "cls" ? val.toFixed(3) : Math.round(val)}<span className="text-xs font-normal text-muted-foreground ms-0.5">{t.unit}</span>
                    </p>
                    <Badge variant={status === "good" ? "default" : status === "needs-improvement" ? "secondary" : "destructive"} className="text-xs mt-1">
                      {status === "good" ? "✓ Good" : status === "needs-improvement" ? "⚠ Needs Work" : "✗ Poor"}
                    </Badge>
                    <p className="text-xs text-muted-foreground mt-1">P75 · ≤{metric === "cls" ? t.good : t.good + t.unit}</p>
                  </>
      ...
      {(() => {
        ...
        if (slowPages.length > 0) return (
          <Card className="border-destructive/30">
            <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-destructive" />{isAr ? "تنبيهات الصفحات البطيئة" : "Slow Page Alerts"}<Badge variant="destructive" className="text-xs ms-auto">{slowPages.length}</Badge></CardTitle></CardHeader>
            <CardContent>
                  ...
                  <div key={alert.path} className="flex items-center gap-3 py-2 px-3 rounded-lg bg-muted/50 border border-border/30">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {[alert.lcp != null && getVitalStatus("lcp", alert.lcp) !== "good" ? `LCP ${Math.round(alert.lcp)}ms` : null, alert.cls != null && getVitalStatus("cls", alert.cls) !== "good" ? `CLS ${alert.cls.toFixed(3)}` : null, alert.inp != null && getVitalStatus("inp", alert.inp) !== "good" ? `INP ${Math.round(alert.inp)}ms` : null].filter(Boolean).join(" · ")}
                      </p>
                    </div>
                  </div>
                ))}
        ...
        if (pageVitals.length > 0) return (
          <Card className="border-chart-2/30 bg-chart-2/5">
            <CardContent className="p-4 flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-chart-2 shrink-0" /><p className="text-sm font-medium">{isAr ? "جميع الصفحات تجتاز حدود Google" : "All pages pass Google's Core Web Vitals thresholds"}</p></CardContent>
          </Card>
        );
        return null;
      })()}
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-border/40 text-xs text-muted-foreground"><th className="text-start py-2 pe-3 font-medium">{isAr ? "الصفحة" : "Page"}</th><th className="text-end py-2 px-2 font-medium">LCP</th><th className="text-end py-2 px-2 font-medium">CLS</th><th className="text-end py-2 px-2 font-medium">INP</th><th className="text-end py-2 ps-2 font-medium">{isAr ? "عينات" : "Samples"}</th></tr></thead>
                <tbody>{pageVitals.slice(0, 20).map(pv => (
                  <tr key={pv.path} className="border-b border-border/20 last:border-0">
                    ...
                    <td className="py-2 px-2 text-end tabular-nums">{pv.lcp != null ? <span className={getVitalStatus("lcp", pv.lcp) === "good" ? "text-chart-2" : getVitalStatus("lcp", pv.lcp) === "needs-improvement" ? "text-chart-4" : "text-destructive"}>{Math.round(pv.lcp)}ms</span> : "—"}</td>
                    <td className="py-2 px-2 text-end tabular-nums">{pv.cls != null ? <span className={getVitalStatus("cls", pv.cls) === "good" ? "text-chart-2" : getVitalStatus("cls", pv.cls) === "needs-improvement" ? "text-chart-4" : "text-destructive"}>{pv.cls.toFixed(3)}</span> : "—"}</td>
                    <td className="py-2 px-2 text-end tabular-nums">{pv.inp != null ? <span className={getVitalStatus("inp", pv.inp) === "good" ? "text-chart-2" : getVitalStatus("inp", pv.inp) === "needs-improvement" ? "text-chart-4" : "text-destructive"}>{Math.round(pv.inp)}ms</span> : "—"}</td>
        ...
        <Card className="border-dashed border-2 border-border/50">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-xs text-muted-foreground mb-4 max-w-md mx-auto">
              {isAr ? "اضغط على زر 'جمع الآن' أعلاه لجمع مقاييس الأداء الحالية، أو انتظر حتى يبدأ الزوار الحقيقيون بالتصفح." : "Click 'Collect Now' above to capture current performance metrics, or wait for real visitors to browse your site. Data will appear automatically."}
            </p>
            <Button onClick={handleCollectVitals} disabled={collectingVitals} size="sm" className="gap-1.5"><Zap className="h-3.5 w-3.5" />{collectingVitals ? (isAr ? "جارٍ الجمع..." : "Collecting...") : (isAr ? "جمع البيانات" : "Collect Performance Data")}</Button>
          </CardContent>
        </Card>
      )}
    </div>
...
// ── Crawlers ──
export const CrawlersSection = memo(function CrawlersSection({ isAr, data }: SectionProps) {
  const { crawlerVisits } = data;
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>{crawlerVisits.slice(0, 20).map((v, i) => (
                  <tr key={i} className="border-b border-border/20 last:border-0"><td className="py-2 pe-3 font-medium">{v.crawler_name}</td><td className="py-2 px-3"><Badge variant="outline" className="text-xs">{v.crawler_type}</Badge></td><td className="py-2 px-3 font-mono text-xs truncate max-w-[200px]">{v.path}</td><td className="py-2 ps-3 text-end text-xs text-muted-foreground">{format(new Date(v.created_at), "dd MMM HH:mm")}</td></tr>
                ))}</tbody>
              </table>
            </div>
          )}
...
// ── Keywords ──
export const KeywordsSection = memo(function KeywordsSection({ isAr, data }: SectionProps) {
  const { trackedKeywords, newKeyword, setNewKeyword, newKeywordPage, setNewKeywordPage, addingKeyword, handleAddKeyword, handleDeleteKeyword, gscSyncing, handleGSCSyncPerformance } = data;
  ...
  return (
    <div className="space-y-4">
                ...
                <tbody>{trackedKeywords.map((kw) => {
                  ...
                  return (
                    <tr key={kw.id} className="border-b border-border/20 last:border-0">
                      <td className="py-2 pe-3"><p className="font-medium">{kw.keyword}</p>{kw.keyword_ar && <p className="text-xs text-muted-foreground">{kw.keyword_ar}</p>}</td>
                      ...
                      <td className="py-2 px-2 text-end text-xs text-muted-foreground">{kw.last_checked_at ? format(new Date(kw.last_checked_at), "dd MMM") : "—"}</td>
      ...
      <div className="flex items-center gap-2 px-1">
        ...
        <span className="text-xs text-muted-foreground">{isAr ? "يتطلب إعداد مفتاح حساب الخدمة" : "Requires Service Account key setup"}</span>
      </div>
    </div>
  );
});
...
// ── Indexing ──
export const IndexingSection = memo(function IndexingSection({ isAr, data }: SectionProps) {
  const { indexingStatus, handleSeedIndexing, gscSyncing, handleGSCInspectUrls, handleGSCSubmitUrls } = data;
    ...
    <div className="space-y-4">
      ...
      {indexingStatus && indexingStatus.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          ].map(s => <Card key={s.label}><CardContent className="p-3 text-center"><p className={`text-xl font-bold ${s.color}`}><AnimatedCounter value={s.count} /></p><p className="text-xs text-muted-foreground">{s.label}</p></CardContent></Card>)}
        </div>
      )}
      <Card>
        <CardContent className="pt-4">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>{indexingStatus.map((page) => (
                  <tr key={page.id} className="border-b border-border/20 last:border-0">
                    ...
                    <td className="py-2 px-2"><Badge variant={page.status === "indexed" ? "default" : page.status === "error" ? "destructive" : "secondary"} className="text-xs">{page.status === "indexed" ? "✓ Indexed" : page.status === "submitted" ? "⏳ Submitted" : page.status === "error" ? "✗ Error" : "? Unknown"}</Badge></td>
                    <td className="py-2 px-2 text-xs text-muted-foreground">{page.submitted_to?.length ? page.submitted_to.join(", ") : "—"}</td>
                    <td className="py-2 ps-2 text-end text-xs text-muted-foreground">{format(new Date(page.updated_at), "dd MMM")}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
...
// ── Top Pages ──
export const PagesSection = memo(function PagesSection({ isAr, data }: SectionProps) {
  const { loadingViews, topPages } = data;
  return (
    <Card>
...
// ── Devices ──
export const DevicesSection = memo(function DevicesSection({ isAr, data }: SectionProps) {
  const { devices, totalViews } = data;
  return (
...
// ── Health ──
export const HealthSection = memo(function HealthSection({ isAr, data }: SectionProps) {
        ...
        <div className="space-y-2">
          {checks.map(check => (
            <div key={check.label} className="flex items-center gap-3 py-2 border-b border-border/20 last:border-0">
              ...
              <div className="flex-1 min-w-0"><p className="text-sm font-medium">{check.label}</p><p className="text-xs text-muted-foreground">{check.detail}</p></div>
              <Badge variant={check.status === "ok" ? "default" : "secondary"} className="text-xs">{check.status === "ok" ? "✓ Pass" : "⚠ Action"}</Badge>
            </div>
          ))}
        </div>
        <div className="mt-6">
          ...
          <div className="flex flex-wrap gap-1.5">{PUBLIC_ROUTES.map(route => {
            ...
            return <Badge key={route.path} variant={hasViews ? "default" : "outline"} className="text-xs gap-1">{hasViews && <CheckCircle2 className="h-2.5 w-2.5" />}{route.label}{hasViews && <span className="opacity-60">{pageCounts[route.path]}</span>}</Badge>;
          })}</div>
        </div>
      </CardContent>
    </Card>
...
// ── Crawl Log ──
export const CrawlLogSection = memo(function CrawlLogSection({ isAr, data }: SectionProps) {
  const { crawlLog } = data;
  return (
    <Card>
        ...
        {!crawlLog?.length ? <div className="text-center py-8 text-muted-foreground text-sm">{isAr ? "لم يتم إرسال إشعارات بعد" : "No pings sent yet"}</div> : (
          <div className="space-y-2">{crawlLog.map((log) => (
            <div key={log.id} className="flex items-center gap-3 py-2 border-b border-border/20 last:border-0">
              ...
              <div className="flex-1 min-w-0"><p className="text-sm font-medium capitalize">{log.search_engine} — {log.action?.replace(/_/g, " ")}</p><p className="text-xs text-muted-foreground">{format(new Date(log.created_at), "dd MMM yyyy HH:mm")}{log.response_code && ` · HTTP ${log.response_code}`}</p></div>
...
// ── URL Health ──
export const UrlHealthSection = memo(function UrlHealthSection({ isAr }: { isAr: boolean }) {
  const redirects = [
  ...
  return (
    <Card>
      <CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><Route className="h-4 w-4 text-primary" />{isAr ? "مراقب صحة الروابط" : "URL Health Monitor"}</CardTitle></CardHeader>
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border/40 text-xs text-muted-foreground"><th className="text-start py-2 pe-3 font-medium">{isAr ? "الرابط القديم" : "Old URL Pattern"}</th><th className="text-start py-2 px-3 font-medium">{isAr ? "الرابط الجديد" : "New URL"}</th><th className="text-start py-2 px-3 font-medium">{isAr ? "النوع" : "Type"}</th><th className="text-start py-2 ps-3 font-medium">{isAr ? "الحالة" : "Status"}</th></tr></thead>
            <tbody>{redirects.map((r, i) => (
              <tr key={i} className="border-b border-border/20 last:border-0">
                ...
                <td className="py-2.5 px-3"><Badge variant={r.type === "Redirect" ? "secondary" : "outline"} className="text-xs">{r.type === "Redirect" ? "301 Redirect" : "Route Update"}</Badge></td>
                <td className="py-2.5 ps-3"><Badge variant="default" className="text-xs gap-1"><CheckCircle2 className="h-2.5 w-2.5" /> {r.status}</Badge></td>
              </tr>
            ))}</tbody>
          </table>
        </div>
        <p className="text-xs text-muted-foreground mt-3">{isAr ? "عمليات إعادة التوجيه النشطة في publicRoutes.tsx — تُعيد توجيه الروابط القديمة تلقائياً مع الحفاظ على ترتيب محركات البحث." : "Active redirects in publicRoutes.tsx — old URLs are automatically redirected preserving SEO equity."}</p>
      </CardContent>
    </Card>
  );
});
...
// ── Sitemap Config ──
export const SitemapConfigSection = memo(function SitemapConfigSection({ isAr, data }: SectionProps) {
  const { sitemapToggles, setSitemapToggles, savingSeo, saveSeoField } = data;
  const sections = [
  ...
  return (
    <Card>
      <CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><Map className="h-4 w-4 text-primary" />{isAr ? "إعدادات خريطة الموقع" : "Sitemap Configuration"}</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <p className="text-xs text-muted-foreground">{isAr ? "اختر الأقسام التي تظهر في خريطة الموقع XML:" : "Choose which sections appear in the XML sitemap:"}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {sections.map(s => (
            <div key={s.key} className="flex items-center justify-between py-2 px-3 rounded-lg border border-border/40 hover:border-primary/20 transition-colors">
              ...
              <Switch checked={sitemapToggles[s.key] !== false} onCheckedChange={(checked) => setSitemapToggles((prev: Record<string, boolean>) => ({ ...prev, [s.key]: checked }))} />
            </div>
          ))}
        </div>
        <Button onClick={() => saveSeoField({ sitemapSections: sitemapToggles })} disabled={savingSeo} size="sm" className="gap-1.5 mt-2">
          <Save className="h-3.5 w-3.5" />{savingSeo ? (isAr ? "جارٍ الحفظ..." : "Saving...") : (isAr ? "حفظ الإعدادات" : "Save Sitemap Settings")}
        </Button>
      </CardContent>
    </Card>
  );
...
// ── robots.txt ──
export const RobotsTxtSection = memo(function RobotsTxtSection({ isAr, data }: SectionProps) {
  const { robotsTxtDraft, setRobotsTxtDraft, savingSeo, saveSeoField } = data;

  return (
    <Card>
      <CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><FileCode className="h-4 w-4 text-primary" />{isAr ? "محرر robots.txt" : "robots.txt Editor"}</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <p className="text-xs text-muted-foreground">{isAr ? "عدّل ملف robots.txt لتتحكم في زحف محركات البحث. يُحفظ هذا المحتوى في قاعدة البيانات." : "Edit the robots.txt content to control search engine crawling. Content is stored in the database."}</p>
        <Textarea value={robotsTxtDraft} onChange={(e) => setRobotsTxtDraft(e.target.value)} className="font-mono text-xs min-h-[320px] bg-muted/30" dir="ltr" />
        <div className="flex items-center gap-2">
          <Button onClick={() => saveSeoField({ robotsTxt: robotsTxtDraft })} disabled={savingSeo} size="sm" className="gap-1.5"><Save className="h-3.5 w-3.5" />{savingSeo ? (isAr ? "جارٍ الحفظ..." : "Saving...") : (isAr ? "حفظ" : "Save robots.txt")}</Button>
          <Button variant="outline" size="sm" className="gap-1.5 text-xs" onClick={() => { setRobotsTxtDraft(DEFAULT_ROBOTS_TXT); toast.info(isAr ? "تم استعادة القيم الافتراضية" : "Reset to defaults"); }}><RefreshCw className="h-3 w-3" />{isAr ? "استعادة الافتراضي" : "Reset to Default"}</Button>
        </div>
      </CardContent>
    </Card>
  );
