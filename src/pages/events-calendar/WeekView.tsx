            ...
            return (
              <div key={i} className={cn("min-h-[380px]", isToday && "bg-primary/3")}>
                <div className={cn(
                  ...
                  <p className="text-xs text-muted-foreground uppercase font-semibold tracking-wider">{format(day, "EEE", isAr ? { locale: arLocale } : undefined)}</p>
                  <p className={cn(
                ...
                <ScrollArea className="h-[320px]">
                  <div className="p-1.5 space-y-1">
                    {dayEvents.length === 0 && (
                      <p className="text-xs text-muted-foreground/15 text-center pt-10">—</p>
                    )}
                    {dayEvents.map(ev => (
