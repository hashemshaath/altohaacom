    ...
    <Card className="shadow-sm border-border/40">
      <div className={cn("flex items-center gap-4 px-5 py-5 border-b border-border/20", isToday && "bg-primary/5")}>
        <div className={cn(
          ...
          <span className="text-xs font-bold uppercase leading-none tracking-wider">{format(currentDate, "EEE", isAr ? { locale: arLocale } : undefined)}</span>
          ...
          {dayEvents.length > 0 && dayEvents.length <= 3 && (
            <div className="mt-1 flex flex-wrap gap-1.5">
              {dayEvents.map(ev => {
                ...
                return (
                  <Badge key={ev.id} variant="outline" className={cn("text-xs px-1.5 py-0 gap-1 border", evColors?.bg, evColors?.text, evColors?.border)}>
    ...
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <Badge variant="outline" className={cn("text-xs px-1.5 py-0 border gap-0.5", colors.bg, colors.text, colors.border)}>
            <IconComp className="h-2.5 w-2.5" />
            {isAr ? label?.ar : label?.en}
          </Badge>
          {!countdown.past && (
            <Badge variant={countdown.urgent ? "destructive" : "secondary"} className="text-xs px-1.5 py-0 gap-0.5">
