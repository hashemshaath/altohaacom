            ...
            <CardContent className="p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                {monthEvents.length > 0 && (
                  <Badge variant={isCurrentMonth ? "default" : "outline"} className="text-xs h-4 px-1.5 tabular-nums">{monthEvents.length}</Badge>
                )}
              </div>
      ...
      {days.map((day, i) => {
        ...
        return (
          <div key={i} className={cn(
            "h-4 flex items-center justify-center text-xs rounded-sm transition-colors",
