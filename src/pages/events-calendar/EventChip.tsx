    ...
    return (
      <div ref={ref} className="max-w-[300px] space-y-2">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <Badge variant="outline" className={cn("text-[11px] px-1.5 py-0 border", colors.bg, colors.text, colors.border)}>
            {isAr ? label?.ar : label?.en}
          </Badge>
          {!countdown.past && (
            <Badge variant={countdown.urgent ? "destructive" : "outline"} className="text-[11px] px-1.5 py-0 gap-0.5">
              <Timer className="h-2 w-2" />{countdown.text}
            </Badge>
          )}
          {event.source !== "global_event" && (
            <Badge variant="secondary" className="text-[11px] px-1 py-0 capitalize">{event.source.replace("_", " ")}</Badge>
          )}
        </div>
        ...
        {event.link && <p className="text-[11px] text-primary font-medium">{isAr ? "اضغط للتفاصيل ←" : "Click for details →"}</p>}
      </div>
    );
  }
);
