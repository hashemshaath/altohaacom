...
export default function AcceptInvite() {
  ...
  const acceptMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: isAr ? "تم قبول الدعوة بنجاح!" : "Invitation accepted successfully!" });
      navigate("/company");
    },
    onError: (err: any) => {
  ...
  const declineMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: isAr ? "تم رفض الدعوة" : "Invitation declined" });
      navigate("/dashboard");
    },
  });
  ...
  if (isLoading || authLoading) {
    return (
      <div className="flex min-h-screen flex-col">
        <Header />
        <main className="container max-w-lg flex-1 py-16">
  ...
  if (!token || !invite) {
    return (
      <div className="flex min-h-screen flex-col">
        <Header />
        <main className="container max-w-lg flex-1 py-16 text-center">
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <SEOHead title={isAr ? "قبول دعوة" : "Accept Invitation"} description="Company employee invitation" />
      <Header />
      <main className="container max-w-lg flex-1 py-12">
