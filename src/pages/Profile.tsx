...
import { User, Edit, Shield, Crown, BarChart3, Wallet, FileText, Gift, Trophy, ShoppingBag, ExternalLink, Link2, Heart, Lock, ShieldCheck, KeyRound } from "lucide-react";
...
export default function Profile() {
  ...
  return (
    ...
    >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4 sm:mt-6 pb-20 md:pb-0">
        {/* Tab Navigation */}
        <div className="sticky top-12 z-30 -mx-4 border-y border-border/20 bg-background/90 px-2 sm:px-3 py-2 sm:py-2.5 backdrop-blur-xl md:rounded-2xl md:border md:border-border/30 md:mx-0 md:px-4 md:bg-card/60 shadow-sm shadow-black/[0.03]">
            ...
            {tabs.map((tab) => {
              ...
              if ((tab as any).href) {
                return (
                  <Link
                    key={tab.id}
                    to={(tab as any).href}
                    className="group relative flex items-center gap-1 sm:gap-1.5 rounded-xl px-2.5 sm:px-3 py-2 typo-tab-text transition-all duration-200 snap-start min-w-max hover:bg-muted/50 text-muted-foreground touch-manipulation active:scale-95"
                  >
              ...
              return (
                <TabsTrigger
                  key={tab.id}
                  value={tab.id}
                  className={`group relative flex items-center gap-1 sm:gap-1.5 rounded-xl px-2.5 sm:px-3 py-2 typo-tab-text transition-all duration-200 snap-start min-w-max data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md data-[state=active]:shadow-primary/15 hover:bg-muted/50 active:scale-95 touch-manipulation ${locked ? "opacity-50" : ""}`}
                >
                  <tab.icon className="h-3.5 w-3.5 shrink-0" />
                  <span>{tab.label}</span>
