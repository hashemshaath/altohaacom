            ...
            <SelectContent>
              {competitions?.map(c => (
                <SelectItem key={c.id} value={c.id}>
                  ...
                  <Badge variant="outline" className="ms-2 text-xs">{c.status.replace("_", " ")}</Badge>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          {EVAL_METHODS.map(m => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground leading-tight">{isAr ? m.desc_ar : m.desc_en}</span>
              </button>
            );
          })}
        </div>
            ...
            <SelectContent>
              {filteredPresets.map(p => (
                <SelectItem key={p.id} value={p.id}>
                  ...
                  <Badge variant="outline" className="ms-2 text-xs">{p.category}</Badge>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="flex items-center justify-between rounded-xl border p-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{isAr ? "إخفاء الأسماء عن الحكام" : "Hide contestant names from judges"}</p>
          </div>
