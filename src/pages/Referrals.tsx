    ...
    <PageShell title={isAr ? "نظام الإحالة" : "Referral Program"} description="Invite friends and earn rewards" seoProps={{ noIndex: true }}>
        ...
        <div className="relative mb-8 overflow-hidden rounded-2xl bg-gradient-to-br from-primary/20 via-background to-chart-2/15 p-6 sm:p-10 border border-primary/10">
          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <Badge variant="outline" className="mb-3 border-primary/20 bg-primary/5 text-primary text-xs uppercase tracking-widest px-3 py-1">
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          {achieved ? (
                            <Badge className="bg-chart-2/20 text-chart-2 text-xs shadow-sm">
                  ...
                  <div className="space-y-3 max-h-[500px] overflow-y-auto">
                    {invitations.map((inv) => {
                      ...
                      const statusBadge = isConverted
                        ? <Badge className="bg-chart-2/20 text-chart-2 border-chart-2/30 text-xs shadow-sm"><CheckCircle2 className="h-2.5 w-2.5 me-0.5" />{isAr ? "مكتمل" : "Converted"}</Badge>
                        : isClicked
                        ? <Badge className="bg-chart-4/20 text-chart-4 border-chart-4/30 text-xs"><MousePointerClick className="h-2.5 w-2.5 me-0.5" />{isAr ? "تم النقر" : "Clicked"}</Badge>
                        : <Badge variant="outline" className="text-xs"><Clock className="h-2.5 w-2.5 me-0.5" />{isAr ? "في الانتظار" : "Pending"}</Badge>;
                      ...
                      return (
                        ...
                        >
                          ...
                          {isConverted && (
                            <div className="mt-2 ms-13 rounded-xl bg-chart-2/5 border border-chart-2/10 px-3 py-2 text-xs text-chart-2">
                          ...
                          {isPending && daysSince >= 7 && (
                            <div className="mt-2 ms-13 rounded-xl bg-chart-4/5 border border-chart-4/10 px-3 py-2 text-xs text-muted-foreground">
