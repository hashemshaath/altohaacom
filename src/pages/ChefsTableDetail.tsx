...
export default function ChefsTableDetail() {
  ...
  const avgScores = scoreLabels.map(s => {
    const scores = submittedEvaluations.map(e => (e as unknown as Record<string, unknown>)[s.key] as number).filter((v) => v != null);
  ...
  const evaluatorData = submittedEvaluations.map((ev, i) => ({
    ...
    scores: scoreLabels.map(s => ({
      name: isAr ? s.ar : s.en,
      score: ((ev as unknown as Record<string, unknown>)[s.key] as number) || 0,
    })),
  }));
                  ...
                  <Card key={ev.id} className="border-border/40">
                    <CardContent className="p-6">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                        {scoreLabels.map(s => {
                          const val = (ev as unknown as Record<string, unknown>)[s.key] as number | null;
