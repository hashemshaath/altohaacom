    ...
    <div className="flex min-h-screen flex-col">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="container relative flex flex-col items-center py-16 text-center sm:py-20 md:py-28">
          <div className="mb-5 rounded-2xl bg-primary/5 p-3 ring-1 ring-primary/10">
      ...
      <section className="border-y bg-card">
        <div className="container grid grid-cols-3 divide-x py-6 sm:py-8">
          {[
            { value: stats?.members || 0, label: isAr ? "عضو مسجل" : "Members" },
            { value: stats?.competitions || 0, label: isAr ? "مسابقة" : "Competitions" },
            { value: stats?.entities || 0, label: isAr ? "جهة معتمدة" : "Entities" },
          ].map((stat) => (
            <div key={stat.label} className="text-center px-2">
              <p className="text-xl font-bold text-primary sm:text-2xl md:text-3xl">{stat.value}+</p>
      ...
      <section className="container py-14 md:py-20">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s) => (
            <Card key={s.title} className="group transition-all hover:shadow-md hover:-translate-y-0.5">
      ...
      <section className="container py-14 md:py-20">
        ...
        <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-6">
          {roles.map((role) => (
            <Card key={role.title} className="text-center transition-all hover:shadow-sm hover:-translate-y-0.5">
              <CardContent className="p-4">
                <div className="mx-auto mb-2.5 flex h-10 w-10 items-center justify-center rounded-full bg-primary/10">
              ...
              {entities.map((entity: any) => {
                ...
                return (
                  <Link key={entity.id} to={`/entities/${entity.slug}`} className="block">
                    <Card className="h-full text-center transition-all hover:shadow-sm hover:-translate-y-0.5">
                      <CardContent className="p-4">
                        {entity.logo_url ? (
                          <img src={entity.logo_url} alt={name} className="mx-auto mb-2.5 h-12 w-12 rounded-lg object-cover" loading="lazy" />
                        ) : (
                          <div className="mx-auto mb-2.5 flex h-12 w-12 items-center justify-center rounded-lg bg-primary/10">
                            <Building2 className="h-6 w-6 text-primary" />
                          </div>
                        )}
                        <h3 className="mb-1 text-xs font-semibold line-clamp-2">{name}</h3>
              ...
              return (
                <Link key={article.id} to={`/news/${article.slug}`} className="group block">
                  <Card className="h-full overflow-hidden transition-all hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <CardContent className="p-4">
                      <h3 className="mb-1.5 line-clamp-2 text-sm font-semibold">{title}</h3>
          ...
          <div className="grid gap-6 grid-cols-2 lg:grid-cols-4">
            {whyUs.map((item) => (
              <div key={item.title} className="flex flex-col items-center text-center">
                <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-primary/10 ring-1 ring-primary/15">
