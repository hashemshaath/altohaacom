              ...
              {filteredItems.map((item, index) => (
                <div key={item.id}>
                  <a href={item.url.startsWith("http") ? item.url : `https://${item.url}`} target="_blank" rel="noopener noreferrer" onClick={() => handleLinkClick(item.id)}
                    ...
                    {item.thumbnail_url && <img src={item.thumbnail_url} alt="" className={`${extra.link_layout === "grid" ? "h-10 w-10" : "h-9 w-9"} rounded-xl object-cover shrink-0 relative z-10`} loading="lazy" />}
