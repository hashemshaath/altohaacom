    ...
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] border border-border/40 bg-card shadow-2xl shadow-black/5">
        ...
        {company.cover_image_url && (
          <img loading="lazy" decoding="async" src={company.cover_image_url} className="absolute inset-0 h-full w-full object-cover opacity-20 grayscale" alt={company.name || "Company cover"} />
        )}
        ...
        <div className="relative flex flex-col gap-8 p-8 md:flex-row md:items-center md:p-12">
          ...
          <div className="min-w-0 flex-1 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Badge variant="outline" className="bg-primary/5 border-primary/20 text-primary font-bold uppercase tracking-widest text-xs px-3 py-1.5">{company.type}</Badge>
              {company.company_number && (
                <Badge variant="outline" className="bg-muted/50 font-mono text-xs px-3 py-1.5">
                  <Hash className="me-1.5 h-3 w-3 text-muted-foreground" />
                  {company.company_number}
                </Badge>
              )}
              {company.country_code && (
                <Badge variant="outline" className="bg-muted/50 font-bold text-xs px-3 py-1.5">
            ...
            {company.operating_countries && company.operating_countries.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-border/40">
                ...
                {(company.operating_countries as string[]).map((cc) => (
                  <Badge key={cc} variant="secondary" className="rounded-xl bg-muted/40 text-xs font-bold px-2 py-0.5">
