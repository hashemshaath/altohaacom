...
import { Megaphone, Eye, MousePointer, DollarSign, Plus, Package, FileText } from "lucide-react";
...
export default function CompanyAdvertising() {
  ...
  const { data: packages = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("ad_packages").select("id, name, name_ar, tier, price, currency, duration_days, description, description_ar, features, max_impressions, max_clicks, max_campaigns, sort_order").eq("is_active", true).order("sort_order").limit(50);
      return data || [];
    },
  });
