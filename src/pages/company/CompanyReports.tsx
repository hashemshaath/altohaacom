...
export default function CompanyReports() {
  ...
  const { totalOrders, totalOrderValue, totalInvoiced, totalPaid, totalPending, totalCredits, totalDebits } = useMemo(() => {
    const tInv = filteredInvoices.reduce((s, i: Record<string, unknown>) => s + (i.amount || 0), 0);
    const tPaid = filteredInvoices.filter((i) => i.status === "paid").reduce((s, i: Record<string, unknown>) => s + (i.amount || 0), 0);
    return {
      totalOrders: filteredOrders.length,
      totalOrderValue: filteredOrders.reduce((s, o: Record<string, unknown>) => s + (o.total_amount || 0), 0),
      totalInvoiced: tInv,
      totalPaid: tPaid,
      totalPending: tInv - tPaid,
      totalCredits: filteredTxns.filter((t) => ["payment", "credit", "refund"].includes(t.type)).reduce((s, t: Record<string, unknown>) => s + (t.amount || 0), 0),
      totalDebits: filteredTxns.filter((t) => ["invoice", "debit"].includes(t.type)).reduce((s, t: Record<string, unknown>) => s + (t.amount || 0), 0),
    };
