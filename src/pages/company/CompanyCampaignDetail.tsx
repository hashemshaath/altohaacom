    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
            <span>{isAr ? "نموذج الفوترة:" : "Billing:"} <Badge variant="outline" className="text-xs">{campaign.billing_model}</Badge></span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { icon: DollarSign, label: isAr ? "المصروف" : "Spent", strValue: formatCurrency(Number((campaign as any).spent || 0), isAr ? "ar" : "en"), color: "text-chart-4" },
      ...
      <Tabs defaultValue="performance">
        <TabsList>
          ...
          <TabsTrigger value="creatives" className="gap-1"><Image className="h-3.5 w-3.5" />{isAr ? "الإعلانات" : "Creatives"} <Badge variant="secondary" className="text-xs ms-1">{creatives.length}</Badge></TabsTrigger>
        </TabsList>
                  ...
                  >
                    ...
                    {c.image_url ? (
                      <div className="relative h-40 bg-muted overflow-hidden">
                        ...
                        <Badge className={`absolute top-2 end-2 text-xs ${statusColors[c.status] || ""}`}>
                    ...
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="rounded-xl bg-muted/50 p-2">
                          <p className="text-xs text-muted-foreground">{isAr ? "مشاهدات" : "Imp."}</p>
                          <AnimatedCounter value={c.impressions || 0} className="text-sm font-bold" format />
                        </div>
                        <div className="rounded-xl bg-muted/50 p-2">
                          <p className="text-xs text-muted-foreground">{isAr ? "نقرات" : "Clicks"}</p>
