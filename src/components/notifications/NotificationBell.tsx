      ...
      <DropdownMenuTrigger asChild>
        <Button ref={triggerRef} variant="ghost" size="icon" className="relative group" title={`${isAr ? "الإشعارات" : "Notifications"} (N)`}>
          {dndMode ? (
            ...
            <Bell className={cn("h-5 w-5 transition-all duration-300 group-hover:rotate-12 group-hover:scale-110tate-12 group-hover:scale-110", unreadCount > 0 && "text-primary")} />
          )}
      ...
      <DropdownMenuContent align="end" className="w-[400px] rounded-2xl border-border/50 shadow-2xl shadow-primary/5 backdrop-blur-sm">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-gradient-to-r from-primary/5 to-transparent rounded-t-xl">
          <div className="flex items-center gap-2">
            ...
            {unreadCount > 0 && (
              <Badge variant="secondary" className="text-xs h-5">{unreadCount} {isAr ? "جديد" : "new"}</Badge>
            )}
          </div>
          ...
          {(Object.keys(categoryLabels) as NotificationCategory[]).map((cat) => {
            ...
            return (
              <Button
                ...
                size="sm"
                className={cn("h-7 text-xs gap-1 px-2.5 rounded-full", category === cat && "shadow-sm")}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative shrink-0 w-9 h-9 mt-0.5">
                      ...
                      <Badge className="absolute -bottom-1 -end-1 h-4 min-w-4 text-xs px-1 justify-center">
                        {count}
                      </Badge>
                    </div>
