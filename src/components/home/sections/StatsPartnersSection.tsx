          ...
          {allLogos.map((item, i) => {
            ...
            return (
              <Wrapper key={`${item.id}-${i}`} {...linkProps} className="shrink-0 flex items-center justify-center" title={item.name}>
                <img loading="lazy" decoding="async" src={item.logo_url} alt={item.name} width={120} height={32} className="h-8 w-auto max-w-[120px] object-contain grayscale opacity-40 transition-all duration-300 hover:grayscale-0 hover:opacity-100" />
              </Wrapper>
            );
          })}
        </div>
