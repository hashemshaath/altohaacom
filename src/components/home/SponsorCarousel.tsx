      ...
      <div className="container">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground max-w-lg mx-auto">
            {isAr ? "شركاؤنا الذين يجعلون المسابقات والفعاليات ممكنة" : "Our partners who make competitions and events possible"}
          </p>
        </div>

        {/* Platinum sponsors - Featured row */}
        {(() => {
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="flex gap-6 py-4" style={{ width: "max-content", animation: `marquee-scroll ${Math.max(20, sponsors.filter(s => s.tier !== "platinum").length * 4)}s linear infinite` }}>
            {[...sponsors.filter(s => s.tier !== "platinum"), ...sponsors.filter(s => s.tier !== "platinum")].map((sponsor, idx) => {
