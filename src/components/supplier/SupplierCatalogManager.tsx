          ...
          <Card key={p.id} className={`rounded-xl overflow-hidden ${!p.is_active ? "opacity-60" : ""}`}>
            ...
            <CardContent className="p-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                ...
                {p.sku && <Badge variant="outline" className="text-xs font-mono">{p.sku}</Badge>}
                {p.in_stock ? (
                  <Badge className="bg-chart-5/10 text-chart-5 border-chart-5/20 text-xs">{isAr ? "متوفر" : "In Stock"}</Badge>
                ) : (
                  <Badge variant="destructive" className="text-xs">{isAr ? "نفد" : "Out"}</Badge>
                )}
                {!p.is_active && <Badge variant="secondary" className="text-xs">{isAr ? "غير نشط" : "Inactive"}</Badge>}
              </div>
              {canEdit && (
