      ...
      <CardContent className="relative p-4 sm:p-5">
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-3">
            ...
            <Badge variant="secondary" className="text-xs font-bold tabular-nums rounded-xl px-2 py-0.5">
          ...
          <div className="space-y-1.5 mb-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <p className="text-xs text-muted-foreground font-medium tabular-nums">{data.percent}%</p>
            </div>
          </div>

          <div className="flex flex-wrap gap-1.5 mb-3">
            {data.missing.slice(0, 3).map(({ key, icon: Icon }) => (
              <span key={key} className="inline-flex items-center gap-1 text-xs text-muted-foreground bg-muted/40 rounded-xl px-2 py-1 border border-border/20 transition-colors hover:border-primary/20 hover:text-foreground">
                <Icon className="h-2.5 w-2.5" />
                {isAr ? missingLabels[key]?.ar : missingLabels[key]?.en}
              </span>
            ))}
            {data.missing.length > 3 && (
              <span className="text-xs text-muted-foreground bg-muted/40 rounded-xl px-2 py-1 border border-border/20">
                +{data.missing.length - 3}
              </span>
            )}
          </div>
