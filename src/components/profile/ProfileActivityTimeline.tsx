...
import { Trophy, Award, GraduationCap, Clock, MessageCircle } from "lucide-react";
    ...
    <Card className="overflow-hidden rounded-2xl border-border/40">
      <div className="border-b border-border/30 bg-muted/20 px-4 py-3">
        <h3 className="flex items-center gap-2.5 text-sm font-semibold">
          ...
          <Badge variant="secondary" className="ms-auto text-xs h-5 px-2">{events.length}</Badge>
        </h3>
      </div>
          ...
          {events.map((event, i) => (
            <div key={event.id} className="relative flex gap-3 pb-4 last:pb-0 group animate-in fade-in-50 slide-in-from-start-2" style={{ animationDelay: `${i * 50}ms` }}>
              ...
              <div className="flex-1 pt-0.5">
                <p className="text-[0.8125rem] leading-snug text-foreground/90 group-hover:text-foreground transition-colors">{event.title}</p>
                <p className="mt-1 text-xs text-muted-foreground font-medium" dir="ltr">
