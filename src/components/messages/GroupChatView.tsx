        ...
        {members.map((m) => (
          <div key={m.user_id} className="flex flex-col items-center gap-0.5 px-1.5">
            <Avatar className="h-7 w-7">
              ...
              <AvatarFallback className="text-xs">{(m.profile?.display_name || m.profile?.full_name || "U")[0].toUpperCase()}</AvatarFallback>
            </Avatar>
                ...
                <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                  <div className="max-w-[75%]">
                    {!isMine && (
                      <div className="flex items-center gap-1.5 mb-1">
                        <Avatar className="h-5 w-5">
                          ...
                          <AvatarFallback className="text-xs">{getSenderName(msg.sender_id)[0]}</AvatarFallback>
                        </Avatar>
                        <span className="text-xs font-medium text-muted-foreground">{getSenderName(msg.sender_id)}</span>
                      </div>
                    )}
                    <div className={`rounded-2xl px-3.5 py-2 ${isMine ? "bg-primary text-primary-foreground rounded-ee-md" : "bg-muted rounded-es-md"}`}>
                      ...
                      <p className={`text-xs mt-1 ${isMine ? "text-primary-foreground/60 text-end" : "text-muted-foreground"}`}>
