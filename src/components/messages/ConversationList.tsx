      ...
      <div className="border-b border-border/30 p-3 sm:p-4 space-y-2.5 sm:space-y-3 bg-gradient-to-b from-primary/5 to-transparent">
        <div className="flex items-center justify-between">
          <h2 className="flex items-center gap-2 sm:gap-2.5 font-black text-sm tracking-tight">
            ...
            {counts.unread > 0 && (
              <Badge className="h-5 min-w-5 text-xs font-black px-1.5 rounded-xl animate-pulse">{counts.unread}</Badge>
            )}
          </h2>
          <div className="flex items-center gap-1">
            {onlineCount > 0 && (
              <Badge variant="outline" className="text-xs gap-1 h-6 border-primary/20 text-primary rounded-xl">
      ...
      <ScrollArea className="flex-1">
        {loadingConversations ? (
          ...
          <div className="p-2 space-y-0.5">
            {chatGroups.length > 0 && (
              <>
                <p className="px-3 pt-2 pb-1 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                ...
                <p className="px-3 pt-1 pb-1 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-2 mt-0.5">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      {conv.unread_count > 0 && (
                        <Badge className="h-5 min-w-5 justify-center text-xs px-1.5 rounded-full font-bold">{conv.unread_count}</Badge>
                      )}
                    </div>
                  </div>
                </div>
