      ...
      {Object.entries(reactions).map(([emoji, userIds]) => {
        ...
        return (
          ...
          >
            ...
            {userIds.length > 1 && (
              <span className="text-xs text-muted-foreground font-medium tabular-nums">{userIds.length}</span>
            )}
          </button>
        );
      })}
      ...
      <Popover open={open} onOpenChange={(v) => { setOpen(v); if (!v) setShowExtended(false); }}>
        ...
        >
          ...
          <button
            ...
            className="w-full mt-1 text-xs text-muted-foreground hover:text-foreground text-center py-1 rounded-md hover:bg-accent transition-colors"
          >
            {showExtended ? "▲" : "▼ more"}
          </button>
        </PopoverContent>
