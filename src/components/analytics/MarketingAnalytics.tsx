          ...
          <Card key={k.label} className={`border-s-4 border-s-${k.color}`}>
            <CardContent className="flex items-center gap-3 py-4">
              ...
              <div>
                ...
                {"subValue" in k && (k as any).subValue > 0 && <p className="text-xs text-muted-foreground"><AnimatedCounter value={Math.round((k as any).subValue)} className="inline" /> SAR {isAr ? "مفقود" : "lost"}</p>}
              </div>
            </CardContent>
          </Card>
        ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <div key={i} className="text-center rounded-xl border p-4 space-y-2">
                ...
                <Badge variant="secondary" className="text-xs"><AnimatedCounter value={typeof step.pct === "number" ? step.pct : 0} className="inline" />%</Badge>
              </div>
            ))}
          </div>
        </CardContent>
                    ...
                    {campaignData.campaigns.slice(0, 8).map((c) => {
                      ...
                      return (
                        <div key={c.id} className="rounded-xl border p-3 space-y-1.5">
                          ...
                          <p className="text-xs text-muted-foreground"><AnimatedCounter value={spendPct} className="inline" />% {isAr ? "من الميزانية" : "of budget spent"}</p>
                        </div>
                      );
                    })}
                  </div>
                ...
                return (
                  <div key={cfg.id} className="rounded-xl border p-3 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground font-mono">{cfg.tracking_id}</p>
                    </div>
