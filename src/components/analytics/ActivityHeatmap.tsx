              ...
              <div className="min-w-[600px]">
                ...
                {days.map((day, dayIdx) => (
                  <div key={dayIdx} className="flex items-center gap-0.5 mb-0.5">
                    <div className="w-12 shrink-0 text-xs text-muted-foreground text-end pe-2">
                      {day}
                    </div>
                    {hours.map((hour) => {
                ...
                <div className="flex items-center justify-end gap-1 mt-3">
                  <span className="text-xs text-muted-foreground me-1">{isAr ? "أقل" : "Less"}</span>
                  {["bg-muted/30", "bg-primary/15", "bg-primary/40", "bg-primary/70", "bg-primary"].map((c, i) => (
                    <div key={i} className={`h-3 w-3 rounded-sm ${c}`} />
                  ))}
                  <span className="text-xs text-muted-foreground ms-1">{isAr ? "أكثر" : "More"}</span>
                </div>
              </div>
            </div>
          )}
