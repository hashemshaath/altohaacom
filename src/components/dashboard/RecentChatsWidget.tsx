    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center gap-2">
          ...
          {unreadCount > 0 && (
            <Badge className="ms-auto text-xs h-4 px-1.5 bg-primary">{unreadCount}</Badge>
          )}
        </CardTitle>
      </CardHeader>
              ...
              <div className={`flex items-center gap-2.5 rounded-xl p-2 transition-all hover:bg-muted/50 active:scale-[0.98] ${chat.isUnread ? "bg-primary/5 border border-primary/10" : ""}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1">
                    ...
                    <span className={`text-xs shrink-0 ${chat.isUnread ? "text-primary font-semibold" : "text-muted-foreground"}`}>
                      {formatDistanceToNow(new Date(chat.created_at), { addSuffix: false })}
                    </span>
                  </div>
                  <p className={`text-xs truncate ${chat.isUnread ? "text-foreground font-medium" : "text-muted-foreground"}`}>
