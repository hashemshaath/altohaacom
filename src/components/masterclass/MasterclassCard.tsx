    ...
    >
      <div className="relative aspect-video shrink-0 overflow-hidden bg-muted">
        ...
        <div className="absolute inset-x-0 top-0 flex items-center justify-between p-4">
          <Badge variant="outline" className={`text-xs ${levelColors[mc.level] || ""}`}>
            {isAr ? levelLabels[mc.level]?.ar : levelLabels[mc.level]?.en}
          </Badge>
          <div className="flex gap-1.5">
            {mc.is_free && (
              <Badge className="bg-chart-2/20 text-chart-2 backdrop-blur-md border-0 text-xs font-black uppercase tracking-wider shadow-lg">
                {isAr ? "مجاني" : "Free"}
              </Badge>
            )}
            {isEnrolled && (
              <Badge className="bg-primary/20 text-primary backdrop-blur-md border-0 text-xs font-black uppercase tracking-wider shadow-lg animate-pulse">
                {isAr ? "مسجل" : "Enrolled"}
              </Badge>
            )}
          </div>
      ...
      <CardContent className="flex flex-1 flex-col p-5">
        ...
        <div className="flex flex-wrap items-center gap-4 text-xs font-bold text-muted-foreground border-t border-border/40 pt-4">
