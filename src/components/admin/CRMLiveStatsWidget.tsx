...
import { Target, Users, TrendingUp, Ticket, MessageSquare, Clock, CheckCircle2 } from "lucide-react";
...
export const CRMLiveStatsWidget = memo(function CRMLiveStatsWidget() {
  const isAr = useIsAr();
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="bg-muted/50 rounded-xl p-2 text-center">
            ...
            <div className="text-xs text-muted-foreground">{isAr ? "متوسط الحل" : "Avg Resolution"}</div>
          </div>
          <div className="bg-muted/50 rounded-xl p-2 text-center">
            <Users className="h-3 w-3 mx-auto mb-1 text-primary" />
            <div className="text-sm font-bold">{data.activeSegments}</div>
            <div className="text-xs text-muted-foreground">{isAr ? "شرائح نشطة" : "Active Segments"}</div>
          </div>
          <div className="bg-muted/50 rounded-xl p-2 text-center">
            <CheckCircle2 className="h-3 w-3 mx-auto mb-1 text-chart-2" />
            <div className="text-sm font-bold">{data.resolvedTickets}</div>
            <div className="text-xs text-muted-foreground">{isAr ? "تم حلها" : "Resolved"}</div>
          </div>
        </div>
      </CardContent>
    </Card>
