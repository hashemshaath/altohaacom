        ...
        <CardContent className="p-3">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.keys(grouped).map(page => {
              ...
              return (
                <Badge key={page} variant="secondary" className="gap-1 text-[10px] rounded-xl">
                  <PageIcon className="h-3 w-3" />
                  {page} ({grouped[page].length})
                </Badge>
              );
            })}
            <Button size="sm" variant="outline" className="h-6 text-[10px] rounded-xl gap-1" asChild>
                  ...
                  <TableRow key={p.id} className={!p.is_active ? "opacity-50" : ""}>
                    <TableCell>
                      <div>
                        ...
                        {p.description && <p className="text-[10px] text-muted-foreground truncate max-w-[180px]">{isAr ? p.description_ar || p.description : p.description}</p>}
                        <code className="text-[9px] text-muted-foreground font-mono">{p.slug}</code>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell><Badge variant="outline" className="text-[10px]">{p.placement_type}</Badge></TableCell>
                    ...
                    <TableCell>{p.is_premium ? <Badge className="text-[9px]">Premium</Badge> : "—"}</TableCell>
                    <TableCell>
