...
import { MessageSquare, Bell, Ticket, Mail, Send, CheckCircle, AlertCircle, Clock, Zap } from "lucide-react";
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from("notifications").select("id", { count: "exact", head: true }).gte("created_at", dayAgo),
        ...
        supabase.from("communication_templates").select("id", { count: "exact", head: true }).eq("is_active", true),
      ]);
