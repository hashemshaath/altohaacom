    ...
    <Card className="group relative overflow-hidden border-border/40 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 hover:border-primary/20 rounded-2xl">
      ...
      <CardContent className="p-4 pt-5">
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          <Badge variant="outline" className={statusColor + " text-xs px-2 py-0.5 rounded-xl"}>
            {user.account_status || "active"}
          </Badge>
          <Badge variant="outline" className="text-xs px-2 py-0.5 rounded-xl">
            {user.account_type === "professional" ? (isAr ? "محترف" : "Pro") : (isAr ? "مستخدم عادي" : "Regular")}
          </Badge>
          {user.membership_tier && user.membership_tier !== "basic" && (
            <Badge variant="outline" className="text-xs px-2 py-0.5 rounded-xl border-chart-4/50 text-chart-4">
              <Crown className="h-2.5 w-2.5 me-0.5" /> {user.membership_tier}
            </Badge>
          )}
          {user.roles?.slice(0, 2).map(r => (
            <Badge key={r.role} variant="outline" className="text-xs px-2 py-0.5 rounded-xl">{r.role}</Badge>
          ))}
          {(user.roles?.length || 0) > 2 && (
            <Badge variant="outline" className="text-xs px-2 py-0.5 rounded-xl">+{(user.roles?.length || 0) - 2}</Badge>
          )}
        </div>
