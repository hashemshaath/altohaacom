...
export const MasterclassInsightsWidget = memo(function MasterclassInsightsWidget() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const r4 = await (supabase as any).from("masterclass_enrollments").select("progress_percentage, completed_at").limit(QUERY_LIMIT_LARGE) as { data: { progress_percentage: number; completed_at: string | null }[] | null };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- non-schema columns
      const r5 = await (supabase as any).from("masterclasses").select("id, title, title_ar, enrollment_count, average_rating").limit(QUERY_LIMIT_LARGE)
