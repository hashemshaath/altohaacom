      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="border-t pt-2 space-y-1.5">
            <p className="text-xs text-muted-foreground font-medium">{isAr ? "التذاكر المفتوحة حسب الأولوية" : "Open by Priority"}</p>
            {Object.entries(data.priorityCount).map(([p, count]) => (
              <div key={p} className="flex items-center justify-between text-xs">
                <span className="capitalize">{p}</span>
                <Badge variant={p === "urgent" ? "destructive" : "outline"} className="text-xs">{count}</Badge>
              </div>
            ))}
          </div>
        </CardContent>
