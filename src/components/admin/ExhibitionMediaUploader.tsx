    ...
    <div className="space-y-4">
      ...
      {coverImageUrl && (
        <div className="space-y-1">
          ...
          <img loading="lazy" decoding="async" src={coverImageUrl} alt="Cover" className="h-24 w-40 rounded-xl object-cover border" />
        </div>
      )}
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2">
          {media.map(item => (
            <div key={item.id} className="group relative rounded-xl border overflow-hidden bg-muted/20">
              <img loading="lazy" decoding="async" src={item.file_url} alt={item.title || ""} className="h-20 w-full object-cover" />
