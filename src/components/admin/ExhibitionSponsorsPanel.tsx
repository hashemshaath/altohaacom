...
import { Plus, Trash2, Award, Building, Search, Pencil, X, Check, AlertTriangle } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="rounded-xl border border-primary/20 bg-primary/[0.02] p-4 space-y-3 animate-in fade-in-0 slide-in-from-top-2 duration-200">
          ...
          {/* Company search */}
          <div>
            <Label className="text-xs text-muted-foreground">{t("Link to existing company (optional)", "ربط بشركة موجودة (اختياري)")}</Label>
              ...
              <div className="mt-1 rounded-lg border bg-popover max-h-28 overflow-y-auto">
                {companies.map(c => (
                  <button key={c.id} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-accent text-start" onClick={() => selectCompany(c)}>
                    {c.logo_url && <img loading="lazy" decoding="async" src={c.logo_url} alt={isAr && c.name_ar ? c.name_ar : c.name} className="h-5 w-5 rounded object-contain" />}
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div><Label className="text-xs">{t("Name (EN)", "الاسم (EN)")} *</Label><Input className="h-8 text-xs" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
            <div><Label className="text-xs">{t("Name (AR)", "الاسم (AR)")}</Label><Input className="h-8 text-xs" value={form.name_ar} onChange={e => setForm(f => ({ ...f, name_ar: e.target.value }))} dir="rtl" /></div>
            <div>
              <Label className="text-xs">{t("Tier", "الفئة")}</Label>
            ...
            <div><Label className="text-xs">{t("Logo URL", "رابط الشعار")}</Label><Input className="h-8 text-xs" value={form.logo_url} onChange={e => setForm(f => ({ ...f, logo_url: e.target.value }))} /></div>
          </div>
          ...
          {sponsors.map(s => {
            ...
            return (
              <div key={s.id} className={cn(
                ...
                {s.logo_url ? (
                  <img loading="lazy" decoding="async" src={s.logo_url} alt={isAr && s.name_ar ? s.name_ar : s.name} className="h-9 w-9 rounded-lg object-contain bg-muted p-0.5 shrink-0" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <Badge className={cn("text-xs h-4 border mt-0.5", tier.color)}>{isAr ? tier.ar : tier.en}</Badge>
                </div>
