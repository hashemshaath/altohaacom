    ...
    <Card className="border-border/40 rounded-2xl">
      <CardHeader className="flex flex-row items-center justify-between pb-3 pt-4 px-5">
        <CardTitle className="text-sm font-semibold flex items-center gap-2">
          ...
          <Badge variant="outline" className="text-xs font-normal rounded-xl tabular-nums">
                      ...
                      {visibleColumns.roles && (
                        <TableCell>
                          <div className="flex flex-wrap gap-1">
                            ...
                            {profile.roles?.slice(0, 3).map((r) => <Badge key={r.role} variant="outline" className="text-xs">{t(r.role as any)}</Badge>)}
                            {(profile.roles?.length || 0) > 3 && <Badge variant="outline" className="text-xs">+{(profile.roles?.length || 0) - 3}</Badge>}
