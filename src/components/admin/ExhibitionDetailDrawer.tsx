  ...
  const { data: analytics } = useQuery({
    ...
    queryFn: async () => {
      const [ticketsRes, reviewsRes, followersRes, boothsRes, sponsorsRes] = await Promise.all([
        supabase.from("exhibition_tickets").select("id, status, created_at, checked_in_at, ticket_type, attendee_name").eq("exhibition_id", exhibitionId!).limit(QUERY_LIMIT_LARGE),
        supabase.from("exhibition_reviews").select("id, rating, content, created_at, user_id, is_verified_attendee, helpful_count").eq("exhibition_id", exhibitionId!).order("created_at", { ascending: false }).limit(QUERY_LIMIT_LARGE),
        supabase.from("exhibition_followers").select("id, created_at").eq("exhibition_id", exhibitionId!).limit(QUERY_LIMIT_LARGE),
        supabase.from("exhibition_booths").select("id, booth_number, status, assigned_to, price, size").eq("exhibition_id", exhibitionId!).limit(QUERY_LIMIT_LARGE),
        supabase.from("exhibition_sponsors").select("id, tier, name, name_ar, logo_url").eq("exhibition_id", exhibitionId!).limit(QUERY_LIMIT_LARGE),
      ]);
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      {(exh as Record<string, unknown>).exhibition_number && (
                        <Badge variant="outline" className="font-mono text-xs h-5">{String((exh as Record<string, unknown>).exhibition_number)}</Badge>
                      )}
                      {derived && (
                      ...
                      <Badge variant="secondary" className="text-xs">{exh.type}</Badge>
                    </div>
                  </div>
                </div>
              </SheetHeader>
                        ...
                        <div className="space-y-1.5">
                          {analytics.sponsors.slice(0, 5).map((sp) => (
                            <div key={sp.id} className="flex items-center gap-2 text-xs p-1.5 rounded bg-muted/30">
                              ...
                              <Badge variant="outline" className="text-xs h-4">{sp.tier}</Badge>
                            </div>
                          ))}
                        </div>
                      </CardContent>
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="space-y-1.5">
                          {analytics.recentTickets.map((tk) => (
                            <div key={tk.id} className="flex items-center justify-between text-xs p-1.5 rounded bg-muted/30">
                              <span className="truncate flex-1">{tk.attendee_name || "—"}</span>
                              <Badge variant={tk.status === "confirmed" ? "default" : "secondary"} className="text-xs h-4">
                        ...
                        <div className="space-y-2">
                          {analytics.recentReviews.map((rv) => (
                            <div key={rv.id} className="p-2.5 rounded-xl bg-muted/30 space-y-1">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-1.5">
                                  {rv.is_verified_attendee && (
                                    <Badge variant="outline" className="text-xs h-4 text-chart-2 border-chart-2/30">
                              ...
                              {rv.content && (
                                <p className="text-xs text-muted-foreground line-clamp-2">{rv.content}</p>
                              )}
                            </div>
                          ))}
                        </div>
