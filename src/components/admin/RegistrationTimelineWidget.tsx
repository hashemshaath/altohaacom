      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center p-2 rounded-xl bg-primary/5">
            ...
            <p className="text-xs text-muted-foreground">{isAr ? "إجمالي التسجيلات" : "Total Registrations"}</p>
          </div>
          ...
          <div className="text-center p-2 rounded-xl bg-chart-4/5">
            ...
            <p className="text-xs text-muted-foreground">{isAr ? "معدل القبول" : "Approval Rate"}</p>
          </div>
        </div>
            ...
            {data.upcoming.map((comp) => {
              ...
              return (
                <div key={comp.id} className="flex items-center justify-between text-xs p-1.5 rounded bg-muted/30">
                  ...
                  {daysLeft !== null && (
                    <Badge variant={daysLeft <= 3 ? "destructive" : "secondary"} className="text-xs px-1.5 py-0 shrink-0 ms-2">
