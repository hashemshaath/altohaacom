                ...
                <Card key={cp.id} className="border-border/40">
                  <CardContent className="p-4">
                    ...
                    <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-3 text-center mb-3">
                      <p className="text-xs text-primary font-bold uppercase">{isAr ? "الإجمالي المقدر" : "Est. Total"} ({cp.estimated_days}d)</p>
                    ...
                    <div className="flex gap-1.5 border-t border-border/30 pt-3">
                      <Button size="sm" variant="outline" className="flex-1 gap-1 h-7 text-xs" onClick={() => setEditing(cp)}>
                        <Edit2 className="h-3 w-3" />{isAr ? "تعديل" : "Edit"}
                      </Button>
                      <Button size="sm" variant="outline" className="gap-1 h-7 text-xs" onClick={() => { setSelectedChefId(cp.chef_id); setActiveSubTab("travel"); }}>
                        <History className="h-3 w-3" />{isAr ? "السجلات" : "Records"}
                      </Button>
                      <Button size="sm" variant="outline" className="h-7 text-xs text-destructive hover:text-destructive" onClick={() => deleteCostProfile.mutate(cp.id)}>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/30">
                    ...
                    <TableHead className="font-bold text-xs uppercase">{isAr ? "الوجهة" : "Destination"}</TableHead>
                    <TableHead className="font-bold text-xs uppercase">{isAr ? "التاريخ" : "Date"}</TableHead>
                    <TableHead className="font-bold text-xs uppercase">{isAr ? "الفندق" : "Hotel"}</TableHead>
                    <TableHead className="font-bold text-xs uppercase">{isAr ? "التأشيرة" : "Visa"}</TableHead>
                    <TableHead className="font-bold text-xs uppercase text-end">{isAr ? "الإجمالي" : "Total"}</TableHead>
