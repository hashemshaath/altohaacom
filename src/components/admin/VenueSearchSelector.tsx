        ...
        <div className="flex items-center gap-3 rounded-xl border border-primary/20 bg-gradient-to-r from-primary/[0.03] to-transparent p-3">
          ...
          {!disabled && (
            <div className="flex gap-1 shrink-0">
              <Button variant="outline" size="sm" className="h-7 text-[10px] rounded-lg gap-1" onClick={() => { setShowResults(true); setSearch(""); }}>
  ...
  return (
    <div className="rounded-xl border border-chart-2/20 bg-chart-2/5 p-4 space-y-3 animate-in fade-in-0 slide-in-from-top-2 duration-200">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div><Label className="text-[11px]">{t("Name (EN)", "الاسم (EN)")} *</Label><Input className="h-8 text-xs" value={form.name} onChange={e => update("name", e.target.value)} /></div>
        <div><Label className="text-[11px]">{t("Name (AR)", "الاسم (AR)")}</Label><Input className="h-8 text-xs" value={form.name_ar} onChange={e => update("name_ar", e.target.value)} dir="rtl" /></div>
      </div>
      <div className="grid gap-3 sm:grid-cols-3">
        <div><Label className="text-[11px]">{t("City", "المدينة")}</Label><Input className="h-8 text-xs" value={form.city} onChange={e => update("city", e.target.value)} /></div>
        <div><Label className="text-[11px]">{t("Country", "الدولة")}</Label><Input className="h-8 text-xs" value={form.country} onChange={e => update("country", e.target.value)} /></div>
        <div><Label className="text-[11px]">{t("Capacity", "السعة")}</Label><Input className="h-8 text-xs" type="number" value={form.capacity} onChange={e => update("capacity", e.target.value)} /></div>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <div><Label className="text-[11px]">{t("Address", "العنوان")}</Label><Input className="h-8 text-xs" value={form.address} onChange={e => update("address", e.target.value)} /></div>
        <div><Label className="text-[11px]">{t("Map URL", "رابط الخريطة")}</Label><Input className="h-8 text-xs" value={form.map_url} onChange={e => update("map_url", e.target.value)} /></div>
      </div>
      <div className="grid gap-3 sm:grid-cols-3">
        <div><Label className="text-[11px]">{t("Phone", "الهاتف")}</Label><Input className="h-8 text-xs" value={form.phone} onChange={e => update("phone", e.target.value)} /></div>
        <div><Label className="text-[11px]">{t("Email", "البريد")}</Label><Input className="h-8 text-xs" value={form.email} onChange={e => update("email", e.target.value)} /></div>
        <div><Label className="text-[11px]">{t("Website", "الموقع")}</Label><Input className="h-8 text-xs" value={form.website} onChange={e => update("website", e.target.value)} /></div>
      </div>
