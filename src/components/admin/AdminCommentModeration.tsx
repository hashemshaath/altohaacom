...
import { Flag, EyeOff, Trash2, Search, Eye, Shield } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card className="p-3 text-center">
          ...
          <p className="text-xs text-muted-foreground">{isAr ? "مبلّغ عنها" : "Flagged"}</p>
        </Card>
        <Card className="p-3 text-center">
          <AnimatedCounter value={hiddenCount} className="text-xl text-muted-foreground" />
          <p className="text-xs text-muted-foreground">{isAr ? "مخفية" : "Hidden"}</p>
        </Card>
      </div>
                ...
                <TableRow key={c.id} className={c.is_hidden ? "opacity-50" : ""}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      {c.is_flagged && <Badge variant="destructive" className="text-xs"><Flag className="h-2.5 w-2.5 me-0.5" />{isAr ? "مبلّغ" : "Flagged"}</Badge>}
                      {c.is_hidden && <Badge variant="secondary" className="text-xs"><EyeOff className="h-2.5 w-2.5 me-0.5" />{isAr ? "مخفي" : "Hidden"}</Badge>}
                      {!c.is_flagged && !c.is_hidden && <Badge variant="outline" className="text-xs">{isAr ? "عادي" : "Normal"}</Badge>}
                    </div>
                  </TableCell>
