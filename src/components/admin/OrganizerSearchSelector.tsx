    ...
    <div className="space-y-2" ref={containerRef}>
      ...
      {value ? (
        <div className="flex items-center gap-2.5 rounded-xl border p-2.5 bg-muted/30">
          {value.logoUrl ? (
            <img loading="lazy" decoding="async" src={value.logoUrl} alt={value.label || "Organizer"} className="h-9 w-9 rounded-md object-cover shrink-0" />
          ) : (
              ...
              {results.length === 0 ? (
                ...
                <div className="p-1 space-y-0.5">
                  {results.map(item => (
                    ...
                    >
                      {item.logoUrl ? (
                        <img loading="lazy" decoding="async" src={item.logoUrl} alt={item.label || "Organizer"} className="h-7 w-7 rounded-md object-cover shrink-0" />
                      ) : (
