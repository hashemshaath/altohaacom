  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from("competitions").select("id", { count: "exact", head: true }).gte("created_at", weekAgo),
        supabase.from("company_orders").select("id", { count: "exact", head: true }),
        supabase.from("company_orders").select("id", { count: "exact", head: true }).gte("created_at", weekAgo),
        ...
        supabase.from("exhibitions").select("id", { count: "exact", head: true }).gte("created_at", weekAgo),
        supabase.from("certificates").select("id", { count: "exact", head: true }),
        supabase.from("certificates").select("id", { count: "exact", head: true }).gte("created_at", weekAgo),
      ]);
    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-xs font-medium">{isAr ? "آخر 7 أيام" : "Last 7 days"}</Badge>
        </div>
      </CardHeader>
          ...
          {modules.map((m, i) => (
            <div key={i} className="p-3 rounded-xl border border-border/30 transition-all duration-200 hover:shadow-[var(--shadow-sm)] hover:-translate-y-0.5 group">
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className="text-xs text-chart-2 font-semibold tabular-nums">
