      ...
      <CardContent className="p-3 space-y-3">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs text-muted-foreground font-medium">{isAr ? "معدل التنفيذ" : "Fulfillment Rate"}</span>
            <span className="text-xs font-semibold tabular-nums">{data.fulfillmentRate}%</span>
          </div>
        ...
        {Object.keys(data.categoryCounts).length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(data.categoryCounts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([cat, count]) => (
              <Badge key={cat} variant="outline" className="text-xs gap-1 font-medium">
