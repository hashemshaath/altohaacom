...
export function useCareerMutations(deps: MutationDeps) {
  ...
  const saveMembershipMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingMembershipId) { const { error } = await supabase.from("entity_memberships").update(payload).eq("id", editingMembershipId); if (error) throw handleSupabaseError(error); }
      else { const { error } = await supabase.from("entity_memberships").insert({ ...payload, user_id: userId, status: "active" }); if (error) throw handleSupabaseError(error); }
    },
  ...
  const addCompetitionMutation = useMutation({
    mutationFn: async () => { const { error } = await supabase.from("competition_registrations").insert({ participant_id: userId, competition_id: selectedCompetitionId, status: "approved" }); if (error) throw handleSupabaseError(error); },
  ...
  const addManualCompetitionMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) { const { error } = await supabase.from("user_career_records").update(payload).eq("id", editingId); if (error) throw handleSupabaseError(error); }
      else { const { error } = await supabase.from("user_career_records").insert(payload); if (error) throw handleSupabaseError(error); }
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["career-records", userId] }); toast({ title: editingId ? (isAr ? "تم التحديث" : "Updated") : (isAr ? "تمت الإضافة" : "Added") }); closeForm(); },
    onError: (err: Error) => toast({ title: isAr ? "خطأ" : "Error", description: err instanceof Error ? err.message : String(err), variant: "destructive" }),
  });
  ...
  const deleteAwardMutation = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("certificates").delete().eq("id", id); if (error) throw handleSupabaseError(error); },
  ...
  const deleteCareerMutation = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("user_career_records").delete().eq("id", id); if (error) throw handleSupabaseError(error); },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["career-records", userId] }); toast({ title: isAr ? "تم الحذف" : "Deleted" }); },
  });

  const deleteMembershipMutation = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("entity_memberships").delete().eq("id", id); if (error) throw handleSupabaseError(error); },
