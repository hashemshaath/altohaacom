...
export function SEOGSCPerformance({ isAr }: Props) {
  ...
  function ChangeIndicator({ current, previous, invert = false }: { current: number; previous: number; invert?: boolean }) {
    ...
    if (diff === 0) return <span className="text-muted-foreground inline-flex items-center gap-0.5 text-[10px]"><Minus className="h-2.5 w-2.5" /> 0%</span>;
    return (
      <span className={`inline-flex items-center gap-0.5 text-[10px] font-medium ${isPositive ? "text-chart-2" : isNegative ? "text-destructive" : "text-muted-foreground"}`}>
        ...
        {[
          ...
          <Card key={i} className="border-border/40 hover:border-primary/20 transition-colors">
            <CardContent className="p-4">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">{isAr ? "آخر 28 يوم" : "Last 28 days"}</p>
            </CardContent>
          </Card>
        ))}
      </div>
                ...
                {topKeywords.map((kw, i) => {
                  ...
                  return (
                    <div key={kw.id} className="flex items-center gap-2 py-1.5 border-b border-border/20 last:border-0">
                      ...
                      {change != null && (
                        <span className={`inline-flex items-center gap-0.5 text-[10px] font-medium ${change > 0 ? "text-chart-2" : change < 0 ? "text-destructive" : "text-muted-foreground"}`}>
