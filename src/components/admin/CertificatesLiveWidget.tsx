...
import { Award, FileText, Send, CheckCircle, Users, LayoutTemplate, TrendingUp } from "lucide-react";
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          ...
          <div className="bg-muted/50 rounded-xl p-2 text-center">
            ...
            <div className="text-xs text-muted-foreground">{isAr ? "معدل الإصدار" : "Issuance Rate"}</div>
          </div>
          <div className="bg-muted/50 rounded-xl p-2 text-center">
            <Users className="h-3 w-3 mx-auto mb-1 text-chart-4" />
            <div className="text-sm font-bold">{data.downloaded}</div>
            <div className="text-xs text-muted-foreground">{isAr ? "التحميلات" : "Downloads"}</div>
          </div>
        </div>
