      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center p-2 rounded-xl bg-muted/50">
            ...
            <p className="text-xs text-muted-foreground">{isAr ? "محكمين نشطين" : "Active Judges"}</p>
          </div>
          <div className="text-center p-2 rounded-xl bg-muted/50">
            <Star className="h-4 w-4 mx-auto text-chart-4 mb-1" />
            <p className="text-lg font-bold">{data?.totalScores || 0}</p>
            <p className="text-xs text-muted-foreground">{isAr ? "تقييمات" : "Scores Given"}</p>
          </div>
          <div className="text-center p-2 rounded-xl bg-muted/50">
            <Trophy className="h-4 w-4 mx-auto text-chart-3 mb-1" />
            <p className="text-lg font-bold">{data?.activeCompetitions || 0}</p>
            <p className="text-xs text-muted-foreground">{isAr ? "مسابقات نشطة" : "Active Comps"}</p>
          </div>
        </div>
              ...
              <div key={comp.id} className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {comp.activeRound && (
                      <Badge variant="secondary" className="text-xs px-1.5 py-0 bg-chart-3/10 text-chart-3">
