...
import {
  ...
  ChevronUp, ChevronDown, LayoutGrid, Layers,
} from "lucide-react";
import {
  type HomepageBlock,
  useHomepageBlocks,
...
export function BlockListManager() {
  ...
  const handleSave = (updates: Partial<HomepageBlock>) => {
    if (editBlock) {
      updateBlock.mutate({ id: editBlock.id, ...updates }, {
        onSuccess: () => {
  ...
  return (
    <>
      ...
      <BlockEditorDrawer
        ...
        onSave={handleSave}
        isPending={createBlock.isPending || updateBlock.isPending}
      />
