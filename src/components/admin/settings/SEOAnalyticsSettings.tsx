...
import {
  BarChart3, Search, Globe, Map, Save, Loader2, Eye, Tag,
} from "lucide-react";
...
export const SEOAnalyticsSettings = memo(function SEOAnalyticsSettings({ settings, onSave, isPending }: Props) {
  ...
  useEffect(() => {
    ...
    if (existing.analytics) setAnalytics(prev => ({ ...prev, ...existing.analytics }));
    if (existing.seo) setSeo(prev => ({ ...prev, ...existing.seo }));
    if (existing.sitemap) setSitemap(prev => ({ ...prev, ...existing.sitemap }));
  }, [JSON.stringify(existing)]);

  const handleSave = () => {
    onSave("seo_analytics", { analytics, seo, sitemap }, "seo");
  };
      ...
      <Card className="rounded-2xl border-border/40">
        <CardHeader className="pb-3">
          ...
          <p className="text-xs text-muted-foreground">
            {isAr ? "أكواد التتبع والتحليلات — تُحمَّل تلقائياً في جميع الصفحات" : "Tracking IDs loaded automatically across all pages"}
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Google Tag Manager ID</Label>
              <Input
                placeholder="GTM-XXXXXXX"
                value={analytics.gtmId}
                onChange={e => setAnalytics(p => ({ ...p, gtmId: e.target.value }))}
                className="font-mono text-sm"
              />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Google Analytics Measurement ID</Label>
              <Input
                placeholder="G-XXXXXXXXXX"
                value={analytics.gaMeasurementId}
                onChange={e => setAnalytics(p => ({ ...p, gaMeasurementId: e.target.value }))}
                className="font-mono text-sm"
              />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Google Search Console Verification</Label>
              <Input
                placeholder="google-site-verification=..."
                value={analytics.gscVerification}
                onChange={e => setAnalytics(p => ({ ...p, gscVerification: e.target.value }))}
                className="font-mono text-sm"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e => setAnalytics(p => ({ ...p, metaPixelId: e.target.value }))}
                className="font-mono text-sm"
              />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">LinkedIn Insight Tag ID</Label>
              <Input
                placeholder="123456"
                value={analytics.linkedinInsightTagId}
                onChange={e => setAnalytics(p => ({ ...p, linkedinInsightTagId: e.target.value }))}
                className="font-mono text-sm"
              />
              <p className="text-[10px] text-muted-foreground">
                {isAr ? "مهم للجمهور المحترف من الطهاة" : "Important for professional chef audience targeting"}
              </p>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium flex items-center gap-1.5">
              ...
              <Input
                ...
                onChange={e => setAnalytics(p => ({ ...p, hotjarSiteId: e.target.value }))}
                className="font-mono text-sm"
              />
            </div>
          </div>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input
                value={seo.siteNameAr}
                onChange={e => setSeo(p => ({ ...p, siteNameAr: e.target.value }))}
                dir="rtl"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                value={seo.siteNameEn}
                onChange={e => setSeo(p => ({ ...p, siteNameEn: e.target.value }))}
              />
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Textarea
                value={seo.defaultTitleAr}
                onChange={e => setSeo(p => ({ ...p, defaultTitleAr: e.target.value }))}
                placeholder="الطهاة | مجتمع الطهاة العالمي — معارض ومسابقات وأكاديميات الطهي"
                rows={2}
                dir="rtl"
                maxLength={80}
              />
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                value={seo.defaultTitleEn}
                onChange={e => setSeo(p => ({ ...p, defaultTitleEn: e.target.value }))}
                placeholder="AlToha | Global Chef Community — Culinary Competitions & Exhibitions"
                rows={2}
                maxLength={80}
              />
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Textarea
                value={seo.defaultDescriptionAr}
                onChange={e => setSeo(p => ({ ...p, defaultDescriptionAr: e.target.value }))}
                placeholder="منصة الطهاة العالمية — اكتشف معارض الطعام والمشروبات، مسابقات الطهي، الجمعيات والأكاديميات الطهية حول العالم. انضم لمجتمع الطهاة المحترفين."
                rows={3}
                dir="rtl"
                maxLength={200}
              />
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                value={seo.defaultDescriptionEn}
                onChange={e => setSeo(p => ({ ...p, defaultDescriptionEn: e.target.value }))}
                placeholder="The global chef community platform. Discover food & beverage exhibitions, cooking competitions, culinary associations and academies worldwide."
                rows={3}
                maxLength={200}
              />
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Textarea
                value={seo.defaultKeywordsAr}
                onChange={e => setSeo(p => ({ ...p, defaultKeywordsAr: e.target.value }))}
                placeholder="طهاة, مجتمع الطهي, معارض الطعام, مسابقات الطهي, أكاديميات الطهي, جمعيات الطهاة, فعاليات الطهي, الطاهي المحترف, طبخ, فنون الطهي"
                rows={2}
                dir="rtl"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea
                value={seo.defaultKeywordsEn}
                onChange={e => setSeo(p => ({ ...p, defaultKeywordsEn: e.target.value }))}
                placeholder="chefs, culinary community, food exhibitions, cooking competitions, culinary academies, chef associations, culinary events, professional chef, gastronomy"
                rows={2}
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={e => setSeo(p => ({ ...p, defaultOgImageUrl: e.target.value }))}
              placeholder="https://altoha.com/og-image.png"
            />
            <p className="text-[10px] text-muted-foreground">
        ...
        <CardContent className="space-y-3">
          {[
            ...
            <div key={item.key} className="flex items-center justify-between rounded-xl border border-border/30 px-4 py-3 hover:bg-muted/30 transition-colors">
              ...
              <Switch
                checked={sitemap[item.key]}
                onCheckedChange={val => setSitemap(p => ({ ...p, [item.key]: val }))}
              />
            </div>
          ))}
        </CardContent>
