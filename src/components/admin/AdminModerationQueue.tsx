    ...
    <Card className={`border-border/50 ${totalItems > 0 ? "border-destructive/20" : ""}`}>
      <CardHeader className="flex flex-row items-center justify-between pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          {totalItems > 0 && (
            <Badge variant="destructive" className="text-xs h-5 px-1.5">{totalItems}</Badge>
          )}
        </CardTitle>
      ...
      <CardContent>
        ...
        {totalItems > 0 && (
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            {(data?.totalPending || 0) > 0 && (
              <Badge className="text-xs bg-chart-4/10 text-chart-4 hover:bg-chart-4/20 border-0">
        ...
        {totalItems === 0 ? (
          <div className="flex flex-col items-center py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground/50 mt-0.5">{isAr ? "آخر فحص: الآن" : "Last checked: just now"}</p>
          </div>
        ) : (
            ...
            {data?.pendingPosts.map((p) => (
              <div key={p.id} className="flex items-center gap-3 rounded-xl border border-chart-4/20 bg-chart-4/5 p-2.5 transition-all duration-200 hover:bg-chart-4/10 hover:shadow-sm group/post">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{format(new Date(p.created_at), "MMM d, HH:mm")}</p>
                </div>
