  ...
  const { data: items } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        supabase.from("culinary_entities").select("id", { count: "exact", head: true }).eq("status", "pending"),
        ...
        supabase.from("chefs_table_requests").select("id", { count: "exact", head: true }).eq("status", "pending"),
        supabase.from("articles").select("id", { count: "exact", head: true }).eq("status", "draft"),
        supabase.from("invoices").select("id", { count: "exact", head: true }).eq("status", "draft"),
        supabase.from("exhibitions").select("id", { count: "exact", head: true }).eq("status", "pending"),
