              ...
              {ALL_ROLES.map(role => {
                ...
                return (
                  <Button key={role} variant={selected ? "default" : "outline"} size="sm"
                    className={`h-7 text-[11px] gap-1 rounded-xl active:scale-[0.98] touch-manipulation ${!selected ? "opacity-50 hover:opacity-80" : ""}`}
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              <Badge variant="secondary" className="text-[10px]">{permissions.length} {t("total", "إجمالي")}</Badge>
            </CardTitle>
