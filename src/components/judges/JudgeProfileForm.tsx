...
const JudgeProfileForm = memo(function JudgeProfileForm({ userId, isAdmin }: Props) {
  ...
  const { data: baseProfile } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select("user_id, full_name, full_name_ar, username, email, phone, avatar_url, bio, bio_ar, specialization, nationality, country_code, city, gender, date_of_birth, years_of_experience, job_title, job_title_ar").eq("user_id", userId).maybeSingle();
      return data;
    },
  });
