...
export const NewsActiveFilters = memo(function NewsActiveFilters({
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 mb-5 animate-in fade-in-50 slide-in-from-top-2 duration-200">
      <span className="text-[10px] text-muted-foreground/60 font-medium uppercase tracking-wider me-1">
      ...
      <button
        onClick={onClearAll}
        className="text-[10px] text-muted-foreground hover:text-destructive transition-colors underline underline-offset-2 ms-1"
      >
...
function FilterChip({ label, icon, onRemove }: { label: string; icon: string; onRemove: () => void }) {
  return (
    <Badge
      variant="secondary"
      className="gap-1 pe-1 ps-2 py-0.5 rounded-lg text-[10px] font-medium bg-primary/10 text-primary border-0 hover:bg-primary/15 transition-colors group cursor-default"
    >
