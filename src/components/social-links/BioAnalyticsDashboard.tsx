...
function TrendBadge({ value }: { value: number }) {
  const isAr = useIsAr();
  if (value === 0) return <Badge variant="secondary" className="text-xs gap-0.5"><Minus className="h-2.5 w-2.5" />0%</Badge>;
  const positive = value > 0;
  return (
    <Badge variant={positive ? "default" : "destructive"} className="text-xs gap-0.5">
      ...
      <div className="flex items-center justify-between">
        ...
        <Tabs value={period} onValueChange={(v) => setPeriod(v as Period)}>
          <TabsList className="h-8">
            <TabsTrigger value="7d" className="text-xs px-2.5 h-6">{isAr ? "7 أيام" : "7D"}</TabsTrigger>
            <TabsTrigger value="14d" className="text-xs px-2.5 h-6">{isAr ? "14 يوم" : "14D"}</TabsTrigger>
            <TabsTrigger value="30d" className="text-xs px-2.5 h-6">{isAr ? "30 يوم" : "30D"}</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground leading-tight truncate">{stat.label}</p>
        ...
        <Card className="border-chart-5/20 bg-gradient-to-r from-chart-5/5 to-transparent">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{isAr ? "مشتركين نشطين" : "Active Subscribers"}</p>
            </div>
              ...
              {funnelData.map((step, i) => {
                ...
                return (
                  <div key={step.stage} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    {i > 0 && <span className="text-xs text-muted-foreground">{convRate}%</span>}
                  </div>
                );
              })}
            </div>
                  ...
                  {linkPerformance.length === 0 ? (
                    ...
                    <TableRow key={item.id} className={item.clicks > 0 ? "" : "opacity-50"}>
                      <TableCell className="text-xs font-bold tabular-nums w-8">
                        {i < 3 && item.clicks > 0 ? (
                          <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-chart-2/15 text-chart-2 text-xs font-bold">{i + 1}</span>
                        ) : (
                      ...
                      <TableCell className="text-xs max-w-[200px]">
                        <div className="flex items-center gap-1.5">
                          ...
                          <div className="min-w-0">
                            ...
                            {item.url && <span className="text-xs text-muted-foreground truncate block max-w-[180px]">{item.url.replace(/^https?:\/\//, "")}</span>}
                          </div>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-xs text-center">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs tabular-nums w-8">{item.share}%</span>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
