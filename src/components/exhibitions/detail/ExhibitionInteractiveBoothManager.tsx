      ...
      <div className="flex gap-2">
        ...
        <div className="flex gap-1">
          {["all", "available", "reserved", "occupied"].map(s => (
            <Button key={s} size="sm" variant={filterStatus === s ? "default" : "outline"} className="text-xs h-9 px-2" onClick={() => setFilterStatus(s)}>
        ...
        <Card key={hallName}>
          <CardHeader className="py-3 px-4">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              {hallName}
              <Badge variant="outline" className="text-xs">{hallBooths.length} {t("booths", "أجنحة")}</Badge>
            </CardTitle>
          </CardHeader>
                    ...
                    <Tooltip key={booth.id}>
                      <TooltipTrigger asChild>
                        ...
                        >
                          ...
                          <span className="text-xs font-bold">{booth.booth_number}</span>
                          <span className="text-xs truncate max-w-full">{isAr && booth.name_ar ? booth.name_ar : booth.name}</span>
                          {booth.price > 0 && (
                            <span className="text-xs font-semibold mt-0.5">{booth.price} {booth.currency}</span>
                          )}
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  {selectedBooth.logo_url && <img src={selectedBooth.logo_url} alt={isAr && selectedBooth.name_ar ? selectedBooth.name_ar : selectedBooth.name} className="h-8 w-8 rounded object-contain" loading="lazy" />}
