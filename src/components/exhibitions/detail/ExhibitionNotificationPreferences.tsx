...
import { Bell, CalendarClock, Ticket, Users, Star, Radio, MessageCircle, LucideIcon } from "lucide-react";
      ...
      <CardHeader className="py-3 px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Label className="text-xs text-muted-foreground">{t("All", "الكل")}</Label>
          ...
          <div key={item.key} className={`flex items-center justify-between px-4 py-2.5 ${i < items.length - 1 ? "border-b border-border/30" : ""}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{isAr ? item.descAr : item.desc}</p>
              </div>
            </div>
            <Switch
