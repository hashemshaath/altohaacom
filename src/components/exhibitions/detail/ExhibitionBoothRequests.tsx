...
export const ExhibitionBoothRequests = memo(function ExhibitionBoothRequests({ exhibitionId, isAr }: Props) {
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      case "approved": return <Badge className="bg-chart-3/10 text-chart-3 text-xs"><CheckCircle2 className="me-1 h-2.5 w-2.5" />{t("Approved", "مقبول")}</Badge>;
      case "rejected": return <Badge className="bg-destructive/10 text-destructive text-xs"><XCircle className="me-1 h-2.5 w-2.5" />{t("Declined", "مرفوض")}</Badge>;
      default: return <Badge variant="outline" className="text-xs"><Clock className="me-1 h-2.5 w-2.5" />{t("Pending", "بانتظار")}</Badge>;
    }
  };
    ...
    <Card>
      <CardHeader className="py-3 px-4">
        <div className="flex items-center justify-between">
          ...
          {pendingCount > 0 && (
            <Badge className="bg-chart-4/10 text-chart-4 text-xs">{pendingCount} {t("pending", "بانتظار")}</Badge>
          )}
        </div>
      </CardHeader>
              ...
              {requests.map((req) => (
                <>
                  <TableRow key={req.id} className={expandedId === req.id ? "bg-muted/30" : ""}>
                    ...
                    <TableCell><Badge variant="secondary" className="text-xs capitalize">{req.preferred_category}</Badge></TableCell>
                    <TableCell className="text-xs capitalize">{req.preferred_size}</TableCell>
                    <TableCell>{statusBadge(req.status)}</TableCell>
                    <TableCell className="text-xs text-muted-foreground">{format(new Date(req.created_at), "MMM d")}</TableCell>
                    <TableCell>
                    ...
                    <TableRow key={`${req.id}-detail`}>
                      <TableCell colSpan={7} className="bg-muted/20 p-4">
                        <div className="space-y-3 max-w-2xl">
                          {req.description && (
                            <div>
                              <p className="text-xs font-medium text-muted-foreground mb-0.5">{t("Description", "الوصف")}</p>
                              <p className="text-xs">{req.description}</p>
                            </div>
                          )}
                          {req.special_requirements && (
                            <div>
                              <p className="text-xs font-medium text-muted-foreground mb-0.5">{t("Special Requirements", "متطلبات خاصة")}</p>
