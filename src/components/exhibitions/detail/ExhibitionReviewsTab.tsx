...
function OrganizerResponse({ review, isAr, isOrganizer, exhibitionId }: { review: any; isAr: boolean; isOrganizer?: boolean; exhibitionId: string }) {
    ...
    return (
      <div className="mt-2 ms-6 rounded-xl p-2.5 border-s-2 bg-primary/5 border-primary/40">
        <div className="flex items-center gap-1.5">
          <Badge variant="secondary" className="text-xs h-3.5 px-1 bg-primary/10 text-primary border-0">
            {isAr ? "رد المنظم" : "Organizer Response"}
          </Badge>
          {review.organizer_response_at && (
            <span className="text-xs text-muted-foreground">{format(new Date(review.organizer_response_at), "MMM d")}</span>
          )}
        </div>
        <p className="text-xs text-muted-foreground mt-1">{review.organizer_response}</p>
      </div>
    );
  }
  ...
  return showInput ? (
    ...
    <button onClick={() => setShowInput(true)} className="mt-2 ms-6 text-xs text-primary hover:underline flex items-center gap-1">
  ...
  return (
    <div className="mt-2 space-y-2">
      {replies.map((r) => {
        ...
        return (
          <div key={r.id} className={`ms-6 rounded-xl p-2.5 border-s-2 ${isOrganizerReply ? "bg-primary/5 border-primary/40" : "bg-muted/40 border-primary/20"}`}>
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-semibold text-foreground">{r.profile?.full_name || r.profile?.username || (isAr ? "مستخدم" : "User")}</span>
              {isOrganizerReply && (
                <Badge variant="secondary" className="text-xs h-3.5 px-1 bg-primary/10 text-primary border-0">
      ...
      {user && (isOrganizer || replies.length > 0) && !showReply && (
        <button onClick={() => setShowReply(true)} className="ms-6 text-xs text-primary hover:underline flex items-center gap-1">
            ...
            <div className="flex-1 p-5 space-y-2">
              {ratingDistribution.map((d) => (
                <div key={d.stars} className="flex items-center gap-2.5">
                  ...
                  <span className="w-6 text-xs text-muted-foreground text-end tabular-nums font-medium">{d.count}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`px-2 py-1 rounded-md text-xs font-medium transition-colors ${filterRating === null ? "bg-primary text-primary-foreground" : "bg-muted/60 text-muted-foreground hover:bg-muted"}`}
            >
              {isAr ? "الكل" : "All"}
            </button>
            {[5, 4, 3, 2, 1].map((r) => (
              <button
                ...
                className={`px-2 py-1 rounded-md text-xs font-medium transition-colors ${filterRating === r ? "bg-primary text-primary-foreground" : "bg-muted/60 text-muted-foreground hover:bg-muted"}`}
              >
                {r}⭐
              </button>
            ))}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {review.is_verified_attendee && (
                      <Badge variant="outline" className="text-xs h-4 px-1.5 text-chart-3 border-chart-3/30 gap-0.5">
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    ...
                    <span className="text-xs text-muted-foreground/60 font-medium">{format(new Date(review.created_at), "MMM d, yyyy")}</span>
                  </div>
