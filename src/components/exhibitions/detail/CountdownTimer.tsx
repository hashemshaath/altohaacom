      ...
      <div className="grid grid-cols-4 gap-2">
        {units.map((u, i) => (
          <div key={i} className="flex flex-col items-center">
            ...
            <span className="mt-1.5 text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground/80">
              {u.label}
            </span>
          </div>
        ))}
    ...
    <div className="flex items-center justify-center gap-3 py-2 sm:gap-6">
      {units.map((u, i) => (
        <div key={i} className="flex flex-col items-center">
          ...
          <span className="mt-2 text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground/80 sm:mt-3 sm:text-xs sm:tracking-[0.2em]">
            {u.label}
          </span>
        </div>
      ))}
