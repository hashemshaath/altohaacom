...
export const ExhibitionStats = memo(forwardRef<HTMLDivElement, Props>(function ExhibitionStats({ exhibitionId, isAr }: Props, _ref) {
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.extra && (
                  <Badge variant="secondary" className="mt-1 text-xs h-4 px-1.5">{item.extra}</Badge>
                )}
              </div>
            );
          })}
