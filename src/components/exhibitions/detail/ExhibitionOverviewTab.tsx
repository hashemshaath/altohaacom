            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {statEntries.map(([key, value]) => (
                <div key={key} className="rounded-xl border p-3 text-center">
                  ...
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{key.replace(/_/g, " ")}</p>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-4 space-y-3">
            {targetedSectors.length > 0 && (
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2">{isAr ? "القطاعات" : "Sectors"}</p>
            ...
            {categories.length > 0 && (
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2">{isAr ? "الفئات" : "Categories"}</p>
                <div className="flex flex-wrap gap-2">
                  ...
                  <Link key={comp.id} to={`/competitions/${comp.id}`} className="flex items-center gap-3 rounded-xl border border-border/60 p-3 hover:bg-primary/5 hover:border-primary/20 transition-all group">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs text-muted-foreground">{format(compStart, "MMM d, yyyy")}</span>
                        {compIsLive && <Badge className="h-4 px-1.5 text-xs bg-destructive text-destructive-foreground border-none">{isAr ? "مباشر" : "LIVE"}</Badge>}
                        {compIsUpcoming && <Badge variant="outline" className="h-4 px-1.5 text-xs border-primary/30 text-primary">{isAr ? "قادم" : "Soon"}</Badge>}
                      </div>
                    </div>
                    <div className="text-end shrink-0">
                      <p className="text-xs font-bold text-primary">{regCount}</p>
                      <p className="text-xs text-muted-foreground">{isAr ? "مسجل" : "entries"}</p>
                    </div>
                  </Link>
                );
              })}
