...
export const ExhibitionVolunteerManager = memo(function ExhibitionVolunteerManager({ exhibitionId, isAr }: Props) {
  ...
  const statusBadge = (s: string) => {
    ...
    return <Badge className={`${m.cls} text-xs gap-0.5`}>{m.icon}{s}</Badge>;
  };
                ...
                {filtered.map((v) => {
                  ...
                  return (
                    <TableRow key={v.id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">{format(new Date(v.created_at), "MMM d")}</TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          {v.status === "pending" && (
                            <>
                              <Button size="sm" variant="outline" className="h-7 text-xs text-chart-3" onClick={() => updateStatus.mutate({ id: v.id, status: "approved" })}>
                                <CheckCircle2 className="h-3 w-3 me-1" />{t("Approve", "قبول")}
                              </Button>
                              <Button size="sm" variant="outline" className="h-7 text-xs text-destructive" onClick={() => updateStatus.mutate({ id: v.id, status: "rejected" })}>
                          ...
                          <Dialog open={taskDialog === v.id} onOpenChange={(o) => { if (!o) setTaskDialog(null); }}>
                            <DialogTrigger asChild>
                              <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setTaskDialog(v.id)}>
