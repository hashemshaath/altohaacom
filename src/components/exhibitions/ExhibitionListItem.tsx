      ...
      <div className="flex gap-4 rounded-2xl border border-border/40 bg-card/60 backdrop-blur-sm p-3 sm:p-4 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:border-primary/30 hover:bg-card hover:-translate-y-0.5">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between">
          <div>
            <div className="flex items-start justify-between gap-2 mb-1">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <Badge className={`${liveStatus.className} text-xs font-black uppercase tracking-wider py-0.5 px-2 border-0`}>
            ...
            {description && (
              <p className="text-xs text-muted-foreground/70 line-clamp-1 leading-relaxed">{description}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            {exhibition.is_free && (
              <Badge variant="outline" className="text-xs text-chart-3 border-chart-3/30 bg-chart-3/5 py-0.5 px-2">
                {isAr ? "مجاني" : "Free"}
              </Badge>
            )}
            {daysLeft !== null && daysLeft > 0 && daysLeft <= 30 && (
              <Badge className="gap-0.5 text-xs bg-chart-4/90 text-chart-4-foreground border-0 py-0.5 px-2">
                <Clock className="h-2.5 w-2.5" />
                {isAr ? `${daysLeft} يوم` : `${daysLeft}d`}
              </Badge>
            )}
            {sponsors.slice(0, 2).map((s) => (
              <Badge key={s.id} variant="outline" className="text-xs bg-chart-4/5 text-chart-4 border-chart-4/20 py-0.5 px-2">
