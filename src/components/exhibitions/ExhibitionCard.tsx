          ...
          >
            ...
            <div className={`relative shrink-0 overflow-hidden bg-muted ${isFeaturedVariant ? "aspect-[16/9]" : "aspect-[16/10]"}`}>
              ...
              <div className="absolute top-3 start-3 z-10 flex flex-col gap-1.5">
                <Badge className={`${liveStatus.className} shadow-lg backdrop-blur-md border-0 text-xs font-black uppercase tracking-wider py-1 px-2.5`}>
                ...
                {exhibition.is_featured && (
                  <Badge className="bg-primary text-primary-foreground shadow-lg shadow-primary/20 text-xs font-black uppercase tracking-wider py-1 px-2.5">
                    ⭐ {isAr ? "مميز" : "Featured"}
                  </Badge>
                )}
              ...
              <div className="absolute top-3 end-3 z-10 flex flex-col items-end gap-1.5">
                <Badge variant="secondary" className="text-xs font-black uppercase tracking-wider bg-background/80 backdrop-blur-md shadow-lg border-0 py-1 px-2.5">
                  {isAr ? typeLabel.ar : typeLabel.en}
                </Badge>
                {daysLeft !== null && daysLeft > 0 && daysLeft <= 30 && (
                  <Badge className="gap-1 text-xs font-black uppercase tracking-wider bg-chart-4/90 text-chart-4-foreground shadow-lg border-0 py-1 px-2.5">
                    <Clock className="h-2.5 w-2.5" />
                    {isAr ? `باقي ${daysLeft} يوم` : `${daysLeft}d left`}
                  </Badge>
                )}
                {regDeadlineDays !== null && regDeadlineDays <= 3 && regDeadlineDays >= 0 && (
                  <Badge className="gap-1 text-xs font-black uppercase tracking-wider bg-destructive/90 text-destructive-foreground shadow-lg border-0 py-1 px-2.5 animate-pulse">
              ...
              <div className="absolute bottom-3 inset-x-3 z-10 flex items-end justify-between">
                {exhibition.country && (
                  <div className="flex items-center gap-1.5 rounded-full bg-background/70 px-2.5 py-1 text-xs font-medium backdrop-blur-md ring-1 ring-border/20">
            ...
            <CardContent className="flex flex-1 flex-col p-4 sm:p-5">
              <div className="flex-1 space-y-2.5">
                ...
                {exhibition.edition_year && (
                  <p className="text-[0.6875rem] font-semibold text-primary/60">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-1.5">
                ...
                {exhibition.is_free ? (
                  <Badge variant="outline" className="text-xs font-bold uppercase tracking-wider text-chart-3 border-chart-3/30 bg-chart-3/5 py-0.5 px-2">
                    {isAr ? "مجاني" : "Free"}
                  </Badge>
                ) : exhibition.ticket_price && (
                  <Badge variant="outline" className="text-xs font-bold border-primary/20 bg-primary/5 py-0.5 px-2">
                    {isAr && exhibition.ticket_price_ar ? exhibition.ticket_price_ar : exhibition.ticket_price}
                  </Badge>
                )}
                {(exhibition.view_count ?? 0) >= 50 && (
                  <Badge variant="outline" className="text-xs font-bold uppercase tracking-wider text-chart-4 border-chart-4/20 bg-chart-4/5 py-0.5 px-2 gap-0.5">
                    <Eye className="h-2 w-2" />
                    {exhibition.view_count}
                  </Badge>
                )}
                {exhibition.tags && exhibition.tags.slice(0, 2).map((tag) => (
                  <Badge key={tag} variant="secondary" className="text-xs bg-muted/50 py-0.5 px-2">#{tag}</Badge>
                ))}
              </div>
            </CardContent>
            ...
            <div className="flex items-center justify-between border-t border-border/30 bg-muted/15 px-4 sm:px-5 py-2.5 transition-all duration-300 group-hover:bg-primary/5 group-hover:border-primary/10">
              ...
              <div className="flex items-center gap-2.5">
                ...
                {(() => {
                  ...
                  return days > 0 ? (
                    <Badge variant="outline" className="text-xs border-border/30 py-0 px-1.5">
