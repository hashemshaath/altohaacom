      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        {[
          ...
          <div key={label} className={`rounded-xl ${bg} p-3 sm:p-4 text-center transition-all hover:scale-[1.02]`}>
            ...
            <p className="text-xs sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mt-0.5">{label}</p>
          </div>
        ))}
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="text-end shrink-0">
                  ...
                  <p className="text-xs text-muted-foreground font-medium">{scorePct}%</p>
                </div>

                {entry.judgesScored >= entry.totalJudges && (
                  <Badge variant="outline" className="text-xs h-5 border-chart-5/30 text-chart-5 shrink-0 rounded-xl hidden sm:flex">
                    ✓ {isAr ? "مكتمل" : "Done"}
                  </Badge>
                )}
              </div>
