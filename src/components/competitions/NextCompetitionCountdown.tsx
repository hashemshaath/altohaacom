      ...
      <Card className="overflow-hidden rounded-2xl border-primary/20 bg-gradient-to-r from-primary/5 via-card to-chart-3/5 transition-all duration-300 hover:shadow-lg hover:shadow-primary/10 hover:-translate-y-0.5">
        <CardContent className="p-4 flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="flex-1 min-w-0 text-center sm:text-start">
            <Badge className="mb-1.5 text-[8px] font-black uppercase tracking-widest bg-primary/10 text-primary border-0 rounded-lg">
            ...
            {(nextComp.city || nextComp.country_code) && (
              <p className="text-[10px] text-muted-foreground flex items-center gap-1 justify-center sm:justify-start mt-0.5">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {units.map((u, i) => (
              <div key={i} className="flex flex-col items-center">
                ...
                <span className="text-[8px] text-muted-foreground font-bold uppercase mt-1">{u.labelShort}</span>
              </div>
            ))}
          </div>
