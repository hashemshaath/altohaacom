          ...
          <div className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                {abbr && <p className="text-xs text-muted-foreground mt-0.5">{abbr}</p>}
                {entityOrganizer?.type && (
                  <p className="text-xs text-muted-foreground capitalize">{entityOrganizer.type.replace("_", " ")}</p>
                )}
              </div>
            </div>
            {entityOrganizer && (
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                {profileOrganizer.is_verified && (
                  <Badge variant="secondary" className="text-xs h-4 px-1 shrink-0">✓</Badge>
                )}
              </div>
              {profileOrganizer.specialization && (
                <p className="text-xs text-muted-foreground truncate mt-0.5">{profileOrganizer.specialization}</p>
              )}
              {profileOrganizer.username && (
                <p className="text-xs text-muted-foreground">@{profileOrganizer.username}</p>
              )}
            </div>
          </div>
          <Button asChild variant="outline" size="sm" className="w-full mt-4 rounded-xl h-9 text-xs font-semibold">
