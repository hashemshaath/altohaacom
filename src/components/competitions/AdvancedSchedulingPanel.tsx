              ...
              {stations.map((station) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground capitalize">{station.status || "available"}</p>
                  </div>
                </div>
              ))}
            </div>
...
function RoundStatusBadge({ status, isAr }: { status: string; isAr: boolean }) {
  ...
  return <Badge variant={s.variant} className="text-xs">{isAr ? s.labelAr : s.label}</Badge>;
}
