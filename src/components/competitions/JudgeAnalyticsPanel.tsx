...
import { BarChart3, AlertTriangle, CheckCircle, TrendingUp, Users, Activity } from "lucide-react";
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{j.scores_count || 0} {isAr ? "تقييم" : "scores"}</p>
                    </div>
                  </div>
                  <Badge variant={biasInfo.variant} className="text-xs">{biasInfo.label}</Badge>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <p className="text-xs text-muted-foreground uppercase">{isAr ? "المتوسط" : "Avg Score"}</p>
                    <p className="text-sm font-bold">{(j.avg_score_given || 0).toFixed(1)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase">{isAr ? "الاتساق" : "Consistency"}</p>
                    <Progress value={(j.consistency_score || 0) * 100} className="h-1.5 mt-1" />
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase">{isAr ? "الانحراف" : "Std Dev"}</p>
