  ...
  const checklistItems = useMemo<ReadinessItem[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        detail: `SAR ${Math.round(sponsoredValue).toLocaleString()} / SAR ${Math.round(totalCost).toLocaleString()} covered`,
        detailAr: `SAR ${Math.round(sponsoredValue).toLocaleString()} / SAR ${Math.round(totalCost).toLocaleString()} مغطى`,
      },
      {
        id: "checklist",
        ...
        <CardContent className="p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-end">
              ...
              <p className="text-xs text-muted-foreground">{completeCount}/{checklistItems.length} {isAr ? "مكتمل" : "complete"}</p>
            </div>
          </div>
            ...
            <Card key={item.id} className={`border ${STATUS_BG[item.status]} transition-colors`}>
              <CardContent className="flex items-center gap-3 p-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{isAr ? item.detailAr : item.detail}</p>
                </div>
