        ...
        <Card className="rounded-2xl border-primary/20">
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
              ...
              <Badge variant="destructive" className="text-xs h-5">{followRequests.length}</Badge>
            </CardTitle>
          </CardHeader>
        ...
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            {newFollowers.length > 0 && (
              <Badge variant="secondary" className="text-xs h-5">{newFollowers.length}</Badge>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <div key={f.user_id} className="group flex flex-col items-center gap-2 p-3 rounded-xl border border-border/30 bg-card/60 backdrop-blur-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 min-w-[130px] w-[130px]">
                    ...
                    <div className="min-w-0 w-full space-y-0.5 text-center">
                      {(() => {
                        ...
                        return (
                          <Link to={`/${f.username || f.user_id}`} className={`font-bold hover:text-primary block transition-colors leading-tight text-center break-words ${(name || "U").length > 14 ? "text-xs" : "text-xs sm:text-sm"}`}>
                            {name}
                          </Link>
                        );
                      })()}
                ...
                <div key={rec.user_id} className="group flex flex-col items-center text-center gap-2 p-3 rounded-xl border border-border/30 bg-card/60 backdrop-blur-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 h-full">
                  ...
                  <div className="min-w-0 w-full flex-1 space-y-0.5">
                    {(() => {
                      ...
                      return (
                        <Link to={`/${rec.username || rec.user_id}`} className={`font-bold hover:text-primary block transition-colors leading-tight text-center break-words ${(name || "U").length > 14 ? "text-xs" : "text-xs sm:text-sm"}`}>
                          {name}
                        </Link>
                      );
                    })()}
