      ...
      <div className={cn("flex", expanded && "flex-1 min-h-0")}>
        ...
        {showOutline && toc.length > 0 && (
          <div className="w-48 shrink-0 border border-t-0 border-e-0 border-border/40 bg-muted/10 overflow-y-auto">
            <p className="px-3 py-2 text-[10px] font-medium text-muted-foreground uppercase tracking-wider border-b border-border/30">
            ...
            <div className="p-1">
              {toc.map((item, i) => (
                <button
                  key={i}
                  className={cn(
                    "w-full text-start px-2 py-1 text-[11px] rounded-lg hover:bg-muted/50 transition-colors truncate flex items-center gap-1",
