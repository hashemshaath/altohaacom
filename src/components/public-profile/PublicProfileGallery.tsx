    ...
    <>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <Badge variant="secondary" className="text-xs h-5 rounded-xl">{mediaFiles.length}</Badge>
            ...
            <Card key={group.name} className="rounded-2xl border-border/25 overflow-hidden">
              <CardContent className="p-4">
                <h3 className="font-semibold text-sm capitalize mb-3 flex items-center gap-2">
                  ...
                  {group.name}
                  <Badge variant="outline" className="text-xs h-4 rounded-md">{group.files.length}</Badge>
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5">
          ...
          {ungrouped.length > 0 && (
            <div>
              <h3 className="font-semibold text-sm mb-3 flex items-center gap-2 text-muted-foreground">
                ...
                <Badge variant="outline" className="text-xs h-4 rounded-md">{ungrouped.length}</Badge>
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5">
                {ungrouped.map((file) => {
                  const globalIdx = allFiles.findIndex((f) => f.name === file.name);
