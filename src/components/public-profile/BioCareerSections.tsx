  ...
  return (
    <div className="mb-4">
      <button type="button" onClick={() => setOpen(v => !v)}
        ...
        <span className="text-xs font-semibold uppercase tracking-[0.2em] group-hover:opacity-80 transition-opacity" style={{ color: theme.accent }}>
          {title}
        </span>
        <span className="text-xs font-medium px-1.5 py-0.5 rounded-full" style={{ background: theme.card, color: theme.textMuted, border: `1px solid ${theme.border}` }}>
          {count}
        </span>
  ...
  return (
    <div className="flex gap-3 px-3.5 py-3 rounded-xl transition-all duration-200 hover:shadow-sm" style={{ background: theme.card, border: `1px solid ${theme.border}` }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {entity && <p className="text-xs mt-0.5 truncate" style={{ color: theme.textMuted }}>{entity}</p>}
        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
          {(start || end) && <Calendar className="h-2.5 w-2.5" style={{ color: theme.textMuted }} />}
          {(start || end) && (
            <span className="text-xs" style={{ color: `${theme.textMuted}99` }}>
              {start}{end && start ? " – " : ""}{end}
            </span>
          )}
          {record.location && (
          ...
          {!record.location && flag && <span className="text-xs">{flag}</span>}
          {host && <span className="text-xs" style={{ color: `${theme.textMuted}cc` }}>🎤 {host}</span>}
          {guest && <span className="text-xs" style={{ color: `${theme.textMuted}cc` }}>👤 {guest}</span>}
          {gpa && <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full" style={{ background: `${theme.accent}10`, color: theme.accent }}>GPA: {gpa}</span>}
          {badges.map((b, i) => (
            <span key={i} className="text-xs font-semibold px-1.5 py-0.5 rounded-full" style={{ background: b.bg, color: theme.accent }}>
              {b.label}
            </span>
          ))}
        </div>
        {desc && (
          <p className="text-xs mt-1.5 leading-relaxed line-clamp-2" dir={isAr ? "rtl" : "ltr"} style={{ color: `${theme.textMuted}cc` }}>
            {desc}
          </p>
        )}
      </div>
  ...
  return (
    <div className="flex gap-3 px-3.5 py-3 rounded-xl transition-all duration-200 hover:shadow-sm" style={{ background: theme.card, border: `1px solid ${theme.border}` }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {title && <p className="text-xs mt-0.5 truncate" style={{ color: theme.textMuted }}>{title}</p>}
        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
          {date && <Calendar className="h-2.5 w-2.5" style={{ color: theme.textMuted }} />}
          {date && <span className="text-xs" style={{ color: `${theme.textMuted}99` }}>{date}</span>}
          {cityCountry && <span className="text-xs" style={{ color: `${theme.textMuted}99` }}>{cityCountry}</span>}
          {typeLabel && (
            <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full" style={{ background: `${theme.accent}15`, color: theme.accent }}>
              {typeLabel}
            </span>
          )}
          {membership.status === "active" && (
            <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full" style={{ background: `${theme.accent}20`, color: theme.accent }}>
              {isAr ? "نشط" : "Active"}
            </span>
          )}
        </div>
  ...
  return (
    <div className="flex gap-3 px-3.5 py-3 rounded-xl transition-all duration-200 hover:shadow-sm" style={{ background: theme.card, border: `1px solid ${theme.border}` }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {achievement && <p className="text-xs mt-0.5 truncate" style={{ color: theme.textMuted }}>{achievement}</p>}
        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
          {date && <Calendar className="h-2.5 w-2.5" style={{ color: theme.textMuted }} />}
          {date && <span className="text-xs" style={{ color: `${theme.textMuted}99` }}>{date}</span>}
          {typeInfo && (
