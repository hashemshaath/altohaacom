        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {record.is_current && (
                <Badge className="bg-chart-3/10 text-chart-3 text-xs h-5">
                  {isAr ? "مستمر" : "Ongoing"}
                </Badge>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-1 text-xs text-muted-foreground">
              ...
              {record.employment_type && (
                <Badge variant="outline" className="text-xs h-4">{record.employment_type}</Badge>
              )}
              {record.education_level && (
                <Badge variant="outline" className="text-xs h-4">{record.education_level}</Badge>
              )}
              ...
              {extraBadges?.map((b, i) => (
                <Badge key={i} variant="outline" className={`text-xs h-4 ${b.className || ""}`}>{b.label}</Badge>
              ))}
            </div>
            {desc && (
              <p className="mt-1.5 text-xs text-muted-foreground leading-relaxed" dir={isAr ? "rtl" : "ltr"}>
                {desc}
              </p>
            )}
          </div>
