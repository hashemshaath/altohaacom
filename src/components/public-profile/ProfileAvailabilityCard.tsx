    ...
    <Card className="rounded-2xl border-chart-2/20 bg-gradient-to-br from-chart-2/5 via-transparent to-chart-2/3 overflow-hidden">
      ...
      <CardContent className="p-4 space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {jobTypes.map((type: string) => (
                <Badge key={type} variant="secondary" className="text-[10px] rounded-lg px-2 py-0.5 capitalize">
                  {type.replace(/_/g, " ")}
                </Badge>
              ))}
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {locations.map((loc: string) => (
                <Badge key={loc} variant="outline" className="text-[10px] rounded-lg px-2 py-0.5">
        ...
        {profile.relocation_preference && (
          <Badge variant="outline" className="text-[10px] rounded-lg border-chart-2/20 text-chart-2 capitalize">
