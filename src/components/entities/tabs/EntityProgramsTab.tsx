...
export const EntityProgramsTab = memo(function EntityProgramsTab({ entityId, onDeleteRequest }: Props) {
  ...
  const { data: programs } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("entity_programs").select("id, name, name_ar, description, program_type, level, duration_months, status, max_students, tuition_fee, start_date, end_date, created_at").eq("entity_id", entityId).order("created_at", { ascending: false }).limit(500);
